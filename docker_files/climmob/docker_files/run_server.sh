...
if [ ! -f /opt/climmob_config/development.ini ]; then
  ...
  mysql -h $MYSQL_HOST_NAME -u $MYSQL_USER_NAME --ssl-mode=DISABLED --password=$MYSQL_USER_PASSWORD climmobv4 < /opt/climmob/docker_files/climmob/docker_files/base_db.sql
fi
