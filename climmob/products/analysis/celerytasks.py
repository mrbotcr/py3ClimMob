...
def createReports(locale, path,user, projectid, data, info, infosheet):
    ...
    pathScript = os.path.join(PATH,"modelo").replace(" ","\ ").replace("(","\(").replace(")","\)")

    print("Rscript "+pathScript+"/climmobv3_analysis21022019.R "+pathInputFiles+"/info.json"+" "+pathInputFiles+"/data.json"+" result.json "+pathouttemp+" TRUE "+infosheet)
    os.system("Rscript "+pathScript+"/climmobv3_analysis21022019.R "+pathInputFiles+"/info.json"+" "+pathInputFiles+"/data.json"+" result.json "+pathouttemp+" TRUE "+infosheet)

    createTheDocuments(locale, path,pathout,pathInputFiles,pathouttemp,"Report",pathouttemp+"/result.json",infosheet,projectid)

    return ""

def createTheDocuments(locale, path,output_dir,pathInputFiles,output_dirtemp,output_fname,result_json,infosheet,projectid):
    ...
    try:
        es = gettext.translation("climmob", localedir=this_file_path, languages=[locale])
        es.install()
        _ = es.gettext
    except:
        locale = 'en'
    ...
    if not os.path.isfile(parts[0]+"/products/analysis/report/report_"+locale+".tex") or not os.path.isfile(parts[0] + "/products/analysis/report/infosheets_" + locale + ".tex"):
        locale = 'en'

    data_general["vars"] = [
        {"var": _("Number of observers"), "val": data_pre[data_pre["Characteristics"][0]]["nobs"][0]},
    ...
    for i in data_pre["Items"]:
        data_general["objs"].append({"techId": i["techName"], "aliasId": i["aliasName"]})
    count = 0
    for i in data_pre["Characteristics"]:
        ...
        for c in data_pre[i]["coeff"]:
            result.append(
                {"obj": c["Item"], "est": c["estimate"], "errEsr": c["SE"], "errCuaStd": c["quasiSE"],
                 "cuaVrnz": c["quasiVar"]})
    ...
    data_general["logos"] = [os.path.join(PATH,"report/prueba.jpg"),os.path.join(PATH,"report/logos.jpg")]
    # infosheets reports
    if infosheet == "TRUE":
        data_by_user = {"users": []}
        ...
        for u in info:
            ...
            for vv in zip(info[u]["table1"]["Item"], info[u]["table1"]["Name"]):  # vars by user
            ...
            for cc in info[u]["table2"]:
                ...
                carac_by_user.append({
                    "carac": carac,
                    "pos": posi
                })
            for pp in info[u]["table3"]:
                positions.append({"pos": "Position " + pp["Position"], "var": pp["Item"]})

            data_by_user["users"].append(
                {
                    "uname": uname, "family": family, "div2": div2, "div1": div1,
    ...
    makeReports(data_general, output_dirtemp, output_fname, 'report_'+locale+'.tex', tmp_tex)  # gen general report
    if infosheet == "TRUE":
        makeReports(data_by_user, output_dirtemp, output_fname + "_INFOS", 'infosheets_'+locale+'.tex', tmp_tex)  # gen info sheets
...
def makeReports(data, output_dir, output_fname, template, input_latex_file):
    ...
    env = Environment(autoescape=False, loader=FileSystemLoader(os.path.join(PATH, 'report')), trim_blocks=False)
    ...
    cmd = "pdflatex -synctex=1 -interaction=nonstopmode --jobname %s -output-directory %s %s" % (
        output_fname, output_dir, input_latex_file.replace(" ","\ ").replace("(","\(").replace(")","\)"))
