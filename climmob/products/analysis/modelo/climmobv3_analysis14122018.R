...
expvar <- df[, as.character(tolower(varNames[,2])) ]
...
geoTRUE <- grepl("farmgoelocation", names(expvar))

## If geolocation info is present, then split the vector into its 
## respectives colunms (lon, lat)
if(any(geoTRUE)){
  geo <- tidyr::separate(expvar, names(expvar[geoTRUE]),
...
results <- vector(mode="list", length = 2+length(char))
names(results) <- c("Items","Characteristics",char)
...
results <- toJSON(results, pretty = TRUE, auto_unbox = TRUE)

#Write json file
write(results, paste0(pathname, "/", outputname))
