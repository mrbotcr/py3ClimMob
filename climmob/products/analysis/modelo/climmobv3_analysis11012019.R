...
## ---------------------------------------------------
## ---------------------------------------------------
#system("R-script, climmobv3_analysis.R info.json data.json r.json output T T")
#Get the arguments
...
plot_nodes <- function(object, font.size = NULL, labels = NULL){
  ...
  xmin <- round(min(do.call(rbind, coeffs)$bmin, na.rm = TRUE) - 0.01, digits = 2)
...
impfields <- lapply(impfields, function(X){names(X) <- paste0("V",1:length(X)); X})
...
if(!is_empty(varNames)){
  varNames <- lapply(varNames, function(X){names(X) <- paste0("V",1:length(X)); X})
...
charNames <- lapply(charNames, function(X){names(X) <- paste0("V",1:length(X)) ; X})
...
for(i in seq_along(char)){
  ...
  treepath <- paste0(getwd(), "/", pathname, "/", char_i,"_tree.svg")
  ...
  #define names
  nodepaths <- paste0(getwd(), "/", pathname, "/", char_i,"_tree", names(plots) ,".svg")
  #put names in a list
