...
def createFieldAgentsReport(locale,url,user, path,projectid, fieldagents):
    ...
    os.system("cp -r '" + os.path.join(PATH,'template','css') + "' '" + pathouttemp+"'")
    os.system("cp -r '" + os.path.join(PATH, 'template', 'img') + "' '" + pathouttemp + "'")
    ...
    for fieldagent in fieldagents:
        odk_settings = {'admin': {"change_server": True, "change_form_metadata": False},
                        'general': {"change_server": True, "navigation": "buttons",
                                    'server_url': url,"username": fieldagent["enum_id"], "password": fieldagent["enum_password"]}}
        ...
        qr_file = os.path.join(pathouttemp+"/img", *[str(fieldagent["enum_id"])+"_"+ str(fieldagent["user_name"]) + ".png"])
        img.save(qr_file)
    ...
    env = Environment(autoescape=False, loader=FileSystemLoader(os.path.join(PATH, 'template')), trim_blocks=False)
    template = env.get_template("app.jinja2")
    render_temp = template.render(data)

    with open(pathouttemp+"/fieldagents_"+projectid+".html", "w") as f:  # saves tex_code to outpout file
        f.write(render_temp)
