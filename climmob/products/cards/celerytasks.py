...
def create_index_html(html,png, qrid,projectname, projectpi, package, projectpiemail, projectpinumber, projectcreationdate, letter):
...
def createCards(path,projectid, packages):
    ...
    veces = 0
    # HTMLS
    os.system("cp -r '" + os.path.join(PATH, 'templates', 'css') + "' '" + pathhtmls + "'")
    os.system("cp -r '" + os.path.join(PATH, 'templates', 'img') + "' '" + pathhtmls + "'")
    ...
    for package in packages:
        ...
        for x in range(0, int(package["project_numcom"])):
            ...
            html = pathhtmls + "/" + str(package["package_id"])+"_"+alphabet[x]+ ".html"
            png = pathpng + "/" + str(package["package_id"]) + "_" + alphabet[x] + ".png"
            qr = pathqr + "/" + str(package["package_id"]) + ".png"
            create_index_html(html,png,qr, package["project_name"],str(package["project_pi"]), "Package "+str(package["package_id"]),str(package["project_piemail"]),"",str(package["project_creationdate"])[0:10], alphabet[x])
