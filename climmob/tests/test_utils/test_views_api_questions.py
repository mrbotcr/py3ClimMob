...
class TestCreateQuestionView(BaseViewTestCase):
    ...
    @patch(
        "climmob.views.Api.questions.languageExistInI18nUser", return_value=False
    )
    ...
    @patch(
        "climmob.views.Api.questions.languageExistInI18nUser", return_value=True
    )
    ...
    def test_process_view_success(
        ...
        self.assertEqual(
            json.loads(response.body.decode()), {"question_data": "data"}
        )
...
class TestUpdateQuestionView(BaseViewTestCase):
    ...
    @patch("climmob.views.Api.questions.categoryExistsById", return_value={"user_name": "test_user"})
    ...
    @patch("climmob.views.Api.questions.getQuestionData", return_value=({'user_name': 'test_user'}, False))
...
class TestDeleteQuestionViewApi(BaseViewTestCase):
    ...
    @patch("climmob.views.Api.questions.getQuestionData")
    def test_process_view_delete_failed(self, mock_get_question_data, mock_delete_question):
...
class TestReadQuestionValuesView(BaseViewTestCase):
    ...
    @patch("climmob.views.Api.questions.getQuestionData", return_value=({"question_dtype": 1}, True))
    ...
    @patch("climmob.views.Api.questions.getQuestionOptions", return_value=[
        {"option_id": 1, "option_text": "Option 1"},
        {"option_id": 2, "option_text": "Option 2"},
    ])
    @patch("climmob.views.Api.questions.getQuestionData", return_value=({"question_dtype": 5, "question_id": "QST123"}, True))
    def test_process_view_success(self, mock_get_question_data, mock_get_question_options):
        response = self.view.processView()
        self.assertEqual(response.status_code, 200)
        expected_response = [
            {"option_id": 1, "option_text": "Option 1"},
    ...
    @patch("climmob.views.Api.questions.getQuestionData", return_value=({"question_dtype": 6, "question_id": "QST123"}, True))
    def test_process_view_success_multiple_selection(self, mock_get_question_data, mock_get_question_options):
        response = self.view.processView()
        self.assertEqual(response.status_code, 200)
        expected_response = [
            {"option_id": 1, "option_text": "Option A"},
...
class TestAddQuestionValueViewApi(BaseViewTestCase):
    ...
    @patch("climmob.views.Api.questions.getQuestionData", return_value=({"question_dtype": 1, "user_name": "test_user"}, True))
    def test_process_view_invalid_question_type(self, mock_get_question_data):
        response = self.view.processView()
        self.assertEqual(response.status_code, 401)
        self.assertIn("This is not a question of type Select one or Multiple selection.", response.body.decode())
        mock_get_question_data.assert_called_once_with(
            "test_user", "QST123", self.view.request
        )

    @patch("climmob.views.Api.questions.getQuestionData", return_value=({"question_dtype": 5, "user_name": "test_user"}, False))
    def test_process_view_question_not_editable(self, mock_get_question_data):
        response = self.view.processView()
        self.assertEqual(response.status_code, 401)
        self.assertIn("This question has already been assigned to a form. You cannot add it.", response.body.decode())
        mock_get_question_data.assert_called_once_with(
            "test_user", "QST123", self.view.request
        )

    @patch("climmob.views.Api.questions.optionExists", return_value=True)
    @patch("climmob.views.Api.questions.getQuestionData", return_value=({"question_dtype": 5, "user_name": "test_user"}, True))
    def test_process_view_option_code_exists(self, mock_get_question_data, mock_option_exists):
        response = self.view.processView()
        self.assertEqual(response.status_code, 401)
        self.assertIn("There is already an option with that code.", response.body.decode())
    ...
    @patch("climmob.views.Api.questions.getQuestionData", return_value=({"question_dtype": 5, "user_name": "test_user"}, True))
    def test_process_view_option_desc_exists(self, mock_get_question_data, mock_option_exists_with_name, mock_option_exists):
        response = self.view.processView()
        self.assertEqual(response.status_code, 401)
        self.assertIn("There is already an option with that description.", response.body.decode())
        mock_option_exists.assert_called_once_with("QST123", "OPT1", self.view.request)
        mock_option_exists_with_name.assert_called_once_with("QST123", "Option 1", self.view.request)

    @patch("climmob.views.Api.questions.addOptionToQuestion", return_value=(False, "Failed to add option"))
    @patch("climmob.views.Api.questions.optionExists", return_value=False)
    @patch("climmob.views.Api.questions.optionExistsWithName", return_value=False)
    @patch("climmob.views.Api.questions.getQuestionData", return_value=({"question_dtype": 5, "user_name": "test_user"}, True))
    def test_process_view_add_option_failure(self, mock_get_question_data, mock_option_exists_with_name, mock_option_exists, mock_add_option):
        response = self.view.processView()
        self.assertEqual(response.status_code, 401)
        self.assertIn("Failed to add option", response.body.decode())
        mock_add_option.assert_called_once_with({'question_id': 'QST123', 'value_code': 'OPT1', 'value_desc': 'Option 1', 'user_name': 'test_user', 'value_isother': 0, 'value_isna': 0}, self.view.request)

    @patch("climmob.views.Api.questions.addOptionToQuestion", return_value=(True, "Option added"))
    ...
    @patch("climmob.views.Api.questions.optionExistsWithName", return_value=False)
    @patch("climmob.views.Api.questions.getQuestionData", return_value=({"question_dtype": 5, "user_name": "test_user"}, True))
    def test_process_view_success(self, mock_get_question_data, mock_option_exists_with_name, mock_option_exists, mock_opcion_other, mock_opcion_na, mock_add_option):
        response = self.view.processView()
        self.assertEqual(response.status_code, 200)
    ...
    @patch("climmob.views.Api.questions.getQuestionData", return_value=({"question_dtype": 5, "user_name": "test_user"}, True))
    def test_process_view_other_option_exists(self, mock_get_question_data, mock_opcion_other):
    ...
    @patch("climmob.views.Api.questions.getQuestionData", return_value=({"question_dtype": 5, "user_name": "test_user"}, True))
    def test_process_view_na_option_exists(self, mock_get_question_data, mock_opcion_na):
        data_with_na = self.valid_data.copy()
        data_with_na["value_isna"] = "1"
        self.view.body = json.dumps(data_with_na)
        response = self.view.processView()
        self.assertEqual(response.status_code, 401)
        self.assertIn("There is already an 'Not applicable' option.", response.body.decode())
    ...
    def test_process_view_option_both_other_and_na(self, mock_get_question_data):
        ...
        response = self.view.processView()
        self.assertEqual(response.status_code, 401)
        self.assertIn("An option cannot be 'Other' and 'Not applicable'.", response.body.decode())

    @patch("climmob.views.Api.questions.getQuestionData", return_value=({"question_dtype": 5, "user_name": "bioversity"}, False))
    def test_process_view_question_from_library(self, mock_get_question_data):
        response = self.view.processView()
        self.assertEqual(response.status_code, 401)
        self.assertIn("The question is from the ClimMob library. You cannot add it.", response.body.decode())
        mock_get_question_data.assert_called_once_with(
            "test_user", "QST123", self.view.request
        )
...
class TestUpdateQuestionValueView(BaseViewTestCase):
    ...
    def test_process_view_question_not_found(self, mock_get_question_data):
        ...
        self.assertEqual(response.status_code, 401)
        self.assertIn("You do not have a question with this ID.", response.body.decode())
        mock_get_question_data.assert_called_once_with("test_user", "QST123", self.view.request)

    @patch("climmob.views.Api.questions.getQuestionData", return_value=({"user_name": "test_user", "question_dtype": 5}, False))
    def test_process_view_question_not_editable(self, mock_get_question_data):
        response = self.view.processView()
        self.assertEqual(response.status_code, 401)
        self.assertIn("This question has already been assigned to a form. You cannot edit it.", response.body.decode())
        mock_get_question_data.assert_called_once_with("test_user", "QST123", self.view.request)

    @patch("climmob.views.Api.questions.getQuestionData", return_value=({"user_name": "test_user", "question_dtype": 1}, True))
    def test_process_view_invalid_question_type(self, mock_get_question_data):
        response = self.view.processView()
        self.assertEqual(response.status_code, 401)
        self.assertIn("This is not a question of type Select one or Multiple selection.", response.body.decode())
    ...
    @patch("climmob.views.Api.questions.optionExists", return_value=False)
    @patch("climmob.views.Api.questions.getQuestionData", return_value=({"user_name": "test_user", "question_dtype": 5}, True))
    def test_process_view_option_not_exists(self, mock_get_question_data, mock_option_exists):
        response = self.view.processView()
        self.assertEqual(response.status_code, 401)
        self.assertIn("Does not have an option with this value_code", response.body.decode())
        mock_get_question_data.assert_called_once_with("test_user", "QST123", self.view.request)
        mock_option_exists.assert_called_once_with("QST123", "OPT1", self.view.request)

    @patch("climmob.views.Api.questions.updateOption", return_value=(False, "Update failed"))
    @patch("climmob.views.Api.questions.getOptionData", return_value={"value_isother": 0, "value_isna": 0})
    @patch("climmob.views.Api.questions.optionExists", return_value=True)
    @patch("climmob.views.Api.questions.getQuestionData", return_value=({"user_name": "test_user", "question_dtype": 5}, True))
    def test_process_view_update_option_failure(self, mock_get_question_data, mock_option_exists, mock_get_option_data, mock_update_option):
        response = self.view.processView()
        self.assertEqual(response.status_code, 401)
        self.assertIn("Update failed", response.body.decode())
        mock_get_question_data.assert_called_once_with("test_user", "QST123", self.view.request)
        mock_option_exists.assert_called_once_with("QST123", "OPT1", self.view.request)
        mock_get_option_data.assert_called_once_with("QST123", "OPT1", self.view.request)
        expected_dataworking = self.valid_data.copy()
        expected_dataworking["value_isother"] = 0
        expected_dataworking["value_isna"] = 0
        mock_update_option.assert_called_once_with(expected_dataworking, self.view.request)

    @patch("climmob.views.Api.questions.updateOption", return_value=(True, "Option updated"))
    @patch("climmob.views.Api.questions.getOptionData", return_value={"value_isother": 0, "value_isna": 0})
    @patch("climmob.views.Api.questions.optionExists", return_value=True)
    @patch("climmob.views.Api.questions.getQuestionData", return_value=({"user_name": "test_user", "question_dtype": 5}, True))
    def test_process_view_success(self, mock_get_question_data, mock_option_exists, mock_get_option_data, mock_update_option):
        response = self.view.processView()
        self.assertEqual(response.status_code, 200)
    ...
    @patch("climmob.views.Api.questions.getQuestionData", return_value=({"user_name": "bioversity", "question_dtype": 5}, False))
    def test_process_view_question_from_library(self, mock_get_question_data):
        response = self.view.processView()
        self.assertEqual(response.status_code, 401)
        self.assertIn("The question is from the ClimMob library. You cannot edit it.", response.body.decode())
        mock_get_question_data.assert_called_once_with("test_user", "QST123", self.view.request)
...
class TestDeleteQuestionValueViewApi(BaseViewTestCase):
    ...
    @patch("climmob.views.Api.questions.optionExists", return_value=False)
    @patch("climmob.views.Api.questions.getQuestionData", return_value=({"question_dtype": 5}, True))
    def test_process_view_option_not_exists(self, mock_get_question_data, mock_option_exists):
        response = self.view.processView()
        self.assertEqual(response.status_code, 401)
        self.assertIn(
        ...
        mock_option_exists.assert_called_once_with(
            "QST123", "OPT1", self.view.request
        )

    @patch("climmob.views.Api.questions.deleteOption", return_value=(False, "Error message"))
    @patch("climmob.views.Api.questions.optionExists", return_value=True)
    @patch("climmob.views.Api.questions.getQuestionData", return_value=({"question_dtype": 5}, True))
    def test_process_view_delete_option_failure(self, mock_get_question_data, mock_option_exists, mock_delete_option):
        response = self.view.processView()
        self.assertEqual(response.status_code, 401)
        ...
        mock_delete_option.assert_called_once_with(
            "QST123", "OPT1", self.view.request
        )
    ...
    @patch("climmob.views.Api.questions.optionExists", return_value=True)
    @patch("climmob.views.Api.questions.getQuestionData", return_value=({"question_dtype": 5}, True))
    def test_process_view_success(self, mock_get_question_data, mock_option_exists, mock_delete_option):
        response = self.view.processView()
        self.assertEqual(response.status_code, 200)
...
class TestUpdateQuestionCharacteristicsView(BaseViewTestCase):
    ...
    @patch("climmob.views.Api.questions.getQuestionData", return_value=({"user_name": "test_user", "question_dtype": 9}, False))
    def test_process_view_question_not_editable(self, mock_get_question_data):
        response = self.view.processView()
        self.assertEqual(response.status_code, 401)
        self.assertIn("This question has already been assigned to a form. You cannot edit it.", response.body.decode())
    ...
    @patch("climmob.views.Api.questions.getQuestionData", return_value=({"user_name": "bioversity", "question_dtype": 9}, False))
    def test_process_view_question_from_library(self, mock_get_question_data):
        response = self.view.processView()
        self.assertEqual(response.status_code, 401)
        self.assertIn("The question is from the ClimMob library. You cannot edit it.", response.body.decode())
    ...
    @patch("climmob.views.Api.questions.updateQuestion", return_value=(False, "Update failed"))
    @patch("climmob.views.Api.questions.getQuestionData", return_value=({"question_dtype": 9}, True))
    def test_process_view_update_failed(self, mock_get_question_data, mock_update_question):
        response = self.view.processView()
        self.assertEqual(response.status_code, 401)
        self.assertIn("Update failed", response.body.decode())
        mock_get_question_data.assert_called_once_with("test_user", "QST123", self.view.request)
        expected_dataworking = self.valid_data.copy()
        mock_update_question.assert_called_once_with(expected_dataworking, self.view.request)
    ...
    def test_process_view_success(self, mock_get_question_data, mock_update_question):
        response = self.view.processView()
        self.assertEqual(response.status_code, 200)
        self.assertIn("You successfully updated the ranking of options.", response.body.decode())
        mock_get_question_data.assert_called_once_with("test_user", "QST123", self.view.request)
        expected_dataworking = self.valid_data.copy()
        mock_update_question.assert_called_once_with(expected_dataworking, self.view.request)
    ...
    def test_process_view_invalid_question_type(self, mock_get_question_data):
        response = self.view.processView()
        self.assertEqual(response.status_code, 401)
        self.assertIn("This is not a question of type ranking of options.", response.body.decode())
        mock_get_question_data.assert_called_once_with("test_user", "QST123", self.view.request)
...
class TestUpdateQuestionPerformanceView(BaseViewTestCase):
    ...
    @patch("climmob.views.Api.questions.getQuestionData", return_value=({"user_name": "test_user", "question_dtype": 10}, False))
    def test_process_view_question_not_editable(self, mock_get_question_data):
        response = self.view.processView()
        self.assertEqual(response.status_code, 401)
        self.assertIn("This question has already been assigned to a form. You cannot edit it.", response.body.decode())
    ...
    @patch("climmob.views.Api.questions.getQuestionData", return_value=({"user_name": "bioversity", "question_dtype": 10}, False))
    def test_process_view_question_from_library(self, mock_get_question_data):
        response = self.view.processView()
        self.assertEqual(response.status_code, 401)
        self.assertIn("The question is from the ClimMob library. You cannot edit it.", response.body.decode())
    ...
    def test_process_view_invalid_question_type(self, mock_get_question_data):
        response = self.view.processView()
        self.assertEqual(response.status_code, 401)
        self.assertIn("This is not a question of type comparison with check.", response.body.decode())
        mock_get_question_data.assert_called_once_with("test_user", "QST123", self.view.request)

    @patch("climmob.views.Api.questions.getQuestionData", return_value=({"question_dtype": 10}, True))
    def test_process_view_missing_wildcard(self, mock_get_question_data):
        ...
        response = self.view.processView()
        self.assertEqual(response.status_code, 401)
        self.assertIn("The comparison with check must have the wildcard {{option}}", response.body.decode())
        mock_get_question_data.assert_called_once_with("test_user", "QST123", self.view.request)
    ...
    def test_process_view_success(self, mock_get_question_data, mock_update_question):
        response = self.view.processView()
        self.assertEqual(response.status_code, 200)
        self.assertIn("You successfully updated the comparison with check.", response.body.decode())
        mock_get_question_data.assert_called_once_with("test_user", "QST123", self.view.request)
        expected_dataworking = self.valid_data.copy()
        mock_update_question.assert_called_once_with(expected_dataworking, self.view.request)
