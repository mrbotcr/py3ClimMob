...
class TestDeleteRegistrySectionView(unittest.TestCase):
    ...
    def test_process_view_get(
        ...
        mock_getRegistryGroupInformation.assert_called_once_with(
            "project_id", "test_group", self.mock_request
        )
    ...
    @patch(
        "climmob.views.registry.deleteRegistryGroup",
    ...
    def test_process_view_delete_failure(
        ...
        mock_deleteRegistryGroup.assert_called_once_with(
            "project_id", "test_group", self.mock_request
        )
...
class TestActionsInSections(unittest.TestCase):
    ...
    @patch(
        "climmob.views.registry.addRegistryGroup",
    ...
    @patch(
        "climmob.views.registry.modifyRegistryGroup", return_value=(False, "repeated")
    )
    ...
    @patch(
        "climmob.views.registry.modifyRegistryGroup",
        return_value=(False, "Some other error"),
    )
...
class TestCancelRegistryView(unittest.TestCase):
    ...
    def test_process_view_post_cancel_registry(
        self,
        ...
        mock_setRegistryStatus.assert_called_once_with(
            "test_user", "test_project", "project_id", 0, self.mock_request
        )
        mock_clean_registry_error_logs.assert_called_once_with(
            self.mock_request, "project_id"
        )
...
class TestGetRegistrySectionView(unittest.TestCase):
    ...
    @patch("climmob.views.registry.projectExists", return_value=True)
    @patch(
        "climmob.views.registry.getTheProjectIdForOwner", return_value="test_project_id"
    )
    @patch(
        "climmob.views.registry.getRegistryGroupInformation",
        return_value={"section_data": "some_data"},
    )
    def test_process_view_success(
        ...
        mock_getRegistryGroupInformation.assert_called_once_with(
            "test_project_id", "test_section", self.mock_request
        )
...
class TestCreateDocumentForm(unittest.TestCase):
    ...
    @patch(
        "climmob.views.registry.getDataFormPreview",
        return_value=(["data"], "finalCloseQst"),
    )
    ...
    def test_create_document_form_with_languages(
        ...
        mock_getDataFormPreview.assert_any_call(
            self.mock_self, self.userOwner, self.projectId, language="en"
        )
        mock_getDataFormPreview.assert_any_call(
            self.mock_self, self.userOwner, self.projectId, language="es"
        )
    ...
    def test_create_document_form_without_languages(
        ...
        mock_getDataFormPreview.assert_called_once_with(
            self.mock_self, self.userOwner, self.projectId, language=self.locale
        )
