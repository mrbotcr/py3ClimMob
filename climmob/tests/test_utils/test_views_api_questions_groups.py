...
class TestCreateGroupOfQuestionView(unittest.TestCase):
    ...
    @patch('climmob.views.Api.questionsGroups.categoryExists', return_value=False)
    def test_process_view_create_add_true(self, mock_categoryExists, mock_addCategory, mock_getCategoryById):
...
class TestUpdateGroupOfQuestionView(unittest.TestCase):
    ...
    @patch('climmob.views.Api.questionsGroups.categoryExistsByUserAndId', return_value=False)
    def test_process_view_no_update_category_no_belong(self, mock_categoryExistsByUserAndId):
        fake_id = str(uuid.uuid4())[-12:]
        self.view.body = json.dumps(
            {
                "qstgroups_name": "Categorized Example",
        ...
        response = self.view.processView()
        self.assertEqual(response.status_code, 401)
        self.assertEqual(response.body, b"You cannot edit this category because it does not belong to your personal library.")

    @patch('climmob.views.Api.questionsGroups.categoryExistsWithDifferentId', return_value=True)
    @patch('climmob.views.Api.questionsGroups.categoryExistsByUserAndId', return_value=True)
    def test_process_view_no_update_category_already_exist(self, mock_categoryExistsByUserAndId, mock_categoryExistsWithDifferentId):
        fake_id = str(uuid.uuid4())[-12:]
        self.view.body = json.dumps(
            {
                "qstgroups_name": "Categorized Example",
        ...
        self.assertEqual(response.body,
                         b"There is already a category with this name.")

    @patch('climmob.views.Api.questionsGroups.updateCategory', return_value=(False,"Error at editing category."))
    @patch('climmob.views.Api.questionsGroups.categoryExistsWithDifferentId', return_value=False)
    @patch('climmob.views.Api.questionsGroups.categoryExistsByUserAndId', return_value=True)
    def test_process_view_no_update_edit_error(self, mock_categoryExistsByUserAndId, mock_categoryExistsWithDifferentId, mock_updateCategory):
        fake_id = str(uuid.uuid4())[-12:]
        self.view.body = json.dumps(
            {
                "qstgroups_name": "Categorized Example",
        ...
        self.assertEqual(response.status_code, 401)
        self.assertEqual(response.body,
                         b"There was a problem updating the category.")

    @patch('climmob.views.Api.questionsGroups.getCategoryById')
    @patch('climmob.views.Api.questionsGroups.updateCategory', return_value=(True, ""))
    @patch('climmob.views.Api.questionsGroups.categoryExistsWithDifferentId', return_value=False)
    @patch('climmob.views.Api.questionsGroups.categoryExistsByUserAndId', return_value=True)
    def test_process_view_update_true(self, mock_categoryExistsByUserAndId, mock_categoryExistsWithDifferentId,
                                               mock_updateCategory, mock_getCategoryById):
...
class TestDeleteGroupOfQuestionView(unittest.TestCase):
    ...
    def test_process_view_no_delete_no_belong(self, mock_categoryExistsByUserAndId):
        ...
        response = self.view.processView()
        self.assertEqual(response.status_code, 401)
        self.assertEqual(response.body, b"You cannot delete this category because it does not belong to your personal library.")

    @patch('climmob.views.Api.questionsGroups.theCategoryHaveQuestions', return_value=True)
    @patch('climmob.views.Api.questionsGroups.categoryExistsByUserAndId', return_value=True)
    def test_process_view_no_delete_have_questions(self, mock_categoryExistsByUserAndId, mock_theCategoryHaveQuestions):
        fake_id = str(uuid.uuid4())[-12:]
        self.view.body = json.dumps({
            "qstgroups_id": fake_id,
        })
        ...
        self.assertEqual(response.status_code, 401)
        self.assertEqual(response.body,
                         b"This category cannot be removed because it has questions.")

    @patch('climmob.views.Api.questionsGroups.deleteCategory', return_value=(False,"Error to delete"))
    @patch('climmob.views.Api.questionsGroups.theCategoryHaveQuestions', return_value=False)
    @patch('climmob.views.Api.questionsGroups.categoryExistsByUserAndId', return_value=True)
    def test_process_view_no_delete_delete_error(self, mock_categoryExistsByUserAndId, mock_theCategoryHaveQuestions, mock_deleteCategory):
        fake_id = str(uuid.uuid4())[-12:]
        self.view.body = json.dumps({
            "qstgroups_id": fake_id,
        })
        ...
        self.assertEqual(response.status_code, 401)
        self.assertEqual(response.body,
                         b"There was a problem removing the category.")

    @patch('climmob.views.Api.questionsGroups.deleteCategory', return_value=(True,""))
    @patch('climmob.views.Api.questionsGroups.theCategoryHaveQuestions', return_value=False)
    @patch('climmob.views.Api.questionsGroups.categoryExistsByUserAndId', return_value=True)
    def test_process_view_delete_true(self, mock_categoryExistsByUserAndId, mock_theCategoryHaveQuestions, mock_deleteCategory):
        fake_id = str(uuid.uuid4())[-12:]
        self.view.body = json.dumps({
            "qstgroups_id": fake_id,
        })
