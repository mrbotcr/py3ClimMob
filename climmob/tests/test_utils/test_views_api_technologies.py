...
class TestCreateTechnologyView(unittest.TestCase):
    ...
    def test_process_view_no_add_all_ready_exist(self, mock_findTechInLibrary):
        ...
        self.assertEqual(response.status_code, 401)
        self.assertEqual(
            response.body, b"This technology already exists in the generic library."
        )

    @patch("climmob.views.Api.technologies.findTechInLibrary")
    def test_process_view_no_add_all_ready_exist_by_user(self, mock_findTechInLibrary):
        self.view.request.method = "POST"
        self.view.body = json.dumps(
            {
                "tech_name": "fake_name",
        ...
        mock_findTechInLibrary.side_effect = [False, True]
        response = self.view.processView()
        self.assertEqual(response.status_code, 401)
        self.assertEqual(
            response.body, b"This technology already exists in your personal library."
        )
    ...
    def test_process_view_add_false(self, mock_findTechInLibrary, mock_addTechnology):
        self.view.request.method = "POST"
        self.view.body = json.dumps(
            {
                "tech_name": "fake_name",
    ...
    @patch(
        "climmob.views.Api.technologies.getTechnologyByName",
        return_value={
            "user_name": "test_user",
            "tech_name": "fake_name",
        },
    ...
    @patch("climmob.views.Api.technologies.findTechInLibrary", return_value=False)
    def test_process_view_add_true(
        self, mock_findTechInLibrary, mock_addTechnology, mock_getTechnologyByName
    ):
        self.view.request.method = "POST"
        self.view.body = json.dumps(
            {
...
class TestUpdateTechnologyView(unittest.TestCase):
    ...
    def test_process_view_no_update_no_param_no_name(self):
        self.view.request.method = "POST"
        self.view.body = json.dumps({"tech_name": "", "tech_id": "123456"})
        response = self.view.processView()
        self.assertEqual(response.status_code, 401)
        self.assertEqual(response.body, b"Not all parameters have data.")
    ...
    def test_process_view_no_update_no_param_no_tech_id(self):
        self.view.request.method = "POST"
        self.view.body = json.dumps({"tech_name": "fake_name", "tech_id": ""})
        response = self.view.processView()
        self.assertEqual(response.status_code, 401)
        self.assertEqual(response.body, b"Not all parameters have data.")

    @patch("climmob.views.Api.technologies.findTechInLibrary")
    def test_process_view_no_update_already_exist_pers_lib(
        self, mock_findTechInLibrary
    ):
        self.view.request.method = "POST"
        self.view.body = json.dumps({"tech_name": "fake_name", "tech_id": "123456"})
        mock_findTechInLibrary.side_effect = [False, True]
        response = self.view.processView()
        self.assertEqual(response.status_code, 401)
        self.assertEqual(
    ...
    def test_process_view_no_update_already_exist(self, mock_findTechInLibrary):
        ...
        self.assertEqual(response.status_code, 401)
        self.assertEqual(
            response.body, b"This technology already exists in " b"the generic library."
        )

    @patch("climmob.views.Api.technologies.getTechnologyByUser", return_value=False)
    @patch("climmob.views.Api.technologies.findTechInLibrary", return_value=False)
    def test_process_view_no_update_no_user_technology(
        self, mock_findTechInLibrary, mock_getTechnologyByUser
    ):
        self.view.request.method = "POST"
        self.view.body = json.dumps({"tech_name": "fake_name", "tech_id": "123456"})
        response = self.view.processView()
    ...
    @patch("climmob.views.Api.technologies.findTechInLibrary", return_value=False)
    def test_process_view_no_update_technology_on_use(
        self,
        ...
        self.assertEqual(
            response.body,
            b"You cannot update this technology because it"
            b" has been assigned to a project.",
        )

    @patch(
        "climmob.views.Api.technologies.updateTechnology", return_value=(False, "Error")
    )
    @patch("climmob.views.Api.technologies.getTechnologyAssigned", return_value=False)
    @patch("climmob.views.Api.technologies.getTechnologyByUser", return_value=True)
    @patch("climmob.views.Api.technologies.findTechInLibrary", return_value=False)
    def test_process_view_no_update_error(
        self,
        mock_findTechInLibrary,
        mock_getTechnologyByUser,
        mock_getTechnologyAssigned,
    ...
    @patch("climmob.views.Api.technologies.getTechnologyByUser", return_value=True)
    @patch("climmob.views.Api.technologies.findTechInLibrary", return_value=False)
    def test_process_view_update_success(
        self,
        mock_findTechInLibrary,
        mock_getTechnologyByUser,
        mock_getTechnologyAssigned,
...
class TestDeleteTechnologyViewAPI(unittest.TestCase):
    ...
    def test_process_view_no_delete_json_param_error(self):
        self.view.request.method = "POST"
        self.view.body = json.dumps({"other": "fake_name", "tech_id": "123456"})
        response = self.view.processView()
        self.assertEqual(response.status_code, 401)
        self.assertEqual(response.body, b"Error in the JSON.")
    ...
    def test_process_view_no_delete_no_user_technology(self, mock_getTechnologyByUser):
        self.view.request.method = "POST"
        self.view.body = json.dumps({"tech_id": "123456"})
        response = self.view.processView()
        self.assertEqual(response.status_code, 401)
        self.assertEqual(response.body, b"You do not have a technology with this ID.")
    ...
    @patch("climmob.views.Api.technologies.getTechnologyAssigned", return_value=True)
    @patch("climmob.views.Api.technologies.getTechnologyByUser", return_value=True)
    def test_process_view_no_delete_technology_on_use(
        self, mock_getTechnologyByUser, mock_getTechnologyAssigned
    ):
        self.view.request.method = "POST"
        self.view.body = json.dumps({"tech_id": "123456"})
        response = self.view.processView()
        ...
        self.assertEqual(
            response.body,
            b"You cannot delete this technology because "
            b"it has been assigned to a project.",
        )

    @patch(
        "climmob.views.Api.technologies.deleteTechnology", return_value=(False, "Error")
    )
    @patch("climmob.views.Api.technologies.getTechnologyAssigned", return_value=False)
    @patch("climmob.views.Api.technologies.getTechnologyByUser", return_value=True)
    def test_process_view_no_delete_error(
        self,
        mock_getTechnologyByUser,
        mock_getTechnologyAssigned,
