...
class TestProjectsSummaryView(ViewBaseTest):
    ...
    def test_projects_summary_view_get_data_product(self):
        ...
        self.assertEqual(result[0]["product_id"], "projectssummary")
    ...
    def test_projects_summary_view_processView_no_post_(
        ...
        mock_path_exists.assert_called_once_with("/mocked/path/_report/projectsSummary_mock.json")
...
class TestDownloadProjectsSummaryView(ViewBaseTest):
    ...
    def test_download_project_summary_view_proces_view_no_admin(self, mock_get_user_info):
        ...
        mock_get_user_info.return_value= {"user_admin":None,}
        with self.assertRaises(HTTPNotFound):
            self.view.processView()
    ...
    @patch("climmob.views.projectsSummary.projectsSummary.os.path.join")
    @patch("climmob.views.projectsSummary.projectsSummary.product_found", return_value = True)
    @patch("climmob.views.projectsSummary.projectsSummary.getProductData")
    @patch("climmob.views.projectsSummary.projectsSummary.getUserInfo")
    def test_download_project_summary_view_proces_view_admin(self, mock_get_user_info, mock_get_product_data,
                                                                mock_product_found, mock_join, mock_file_response):
        self.view.request.matchdict["celery_taskid"] = "some_value_celery_taskid"
        self.view.request.matchdict["product_id"] = "some_value_product_id"
        self.view.request.registry.settings["user.repository"] = "/mocked/path/"
        mock_get_user_info.return_value= {"user_admin":1,}
        ...
        self.assertEqual(response.content_disposition,'attachment; filename="projectsSummary_mock.json"')
        mock_get_user_info.asser_called_once(self.view.request, self.view.user.login)
        mock_get_product_data.assert_called_once_with(None, self.view.request.matchdict["celery_taskid"],
                                                      self.view.request.matchdict["product_id"], self.view.request )
        mock_product_found.assert_called_once_with(mock_get_product_data.return_value["product_id"])
        mock_join.assert_called_once_with(self.view.request.registry.settings["user.repository"],"_report", mock_get_product_data.return_value["output_id"] )
        mock_file_response.assert_called_once_with(mock_join.return_value,request=self.view.request,content_type="application/sheet")


    @patch("climmob.views.projectsSummary.projectsSummary.product_found", return_value=False)
    @patch("climmob.views.projectsSummary.projectsSummary.getProductData")
    @patch("climmob.views.projectsSummary.projectsSummary.getUserInfo")
    def test_download_project_summary_view_proces_view_admin_no_prodict\
                    (self, mock_get_user_info, mock_get_product_data,mock_product_found,):
        self.view.request.matchdict["celery_taskid"] = "some_value_celery_taskid"
        self.view.request.matchdict["product_id"] = "some_value_product_id"
        self.view.request.registry.settings["user.repository"] = "/mocked/path/"
        mock_get_user_info.return_value = {"user_admin": 1, }
...
class TestProjectsSummaryCurationView(ViewBaseTest):
    ...
    @patch("climmob.views.projectsSummary.projectsSummary.get_project_summary_columns")
    def test_projects_summary_curation_view_process_view_admin(self, mock_get_project_summary_columns, mock_get_all_project_summary):
        self.view.user.admin = 1
        mock_get_project_summary_columns.return_value = {"column1":"column1",}
        mock_get_all_project_summary.return_value = {"data1":"data1"}

        response = self.view.processView()
        self.assertEqual(response, {"tableStructure":mock_get_project_summary_columns.return_value,
