...
###
        ...
        @patch('climmob.views.cleanErrorLogs.getProjectData', return_value={"project_regstatus": "1"})
        @patch('climmob.views.cleanErrorLogs.isAssessmentOpen', return_value=True)
        @patch('climmob.views.cleanErrorLogs.convertJsonLog', return_value={"converted_key": "converted_value"})
        @patch('builtins.open', new_callable=mock_open, read_data='{"key": "value"}')
        def test_processView_logid_present(self, mock_open_func, mock_convertJsonLog, mock_isAssessmentOpen,
                                           mock_getProjectData, mock_getTheProjectIdForOwner, mock_projectExists):
            """
        ...
        @patch('climmob.views.cleanErrorLogs.getProjectData', return_value={"project_regstatus": "1"})
        @patch('climmob.views.cleanErrorLogs.isAssessmentOpen', return_value=True)
        def test_processView_logid_not_present(self, mock_isAssessmentOpen, mock_getProjectData,
...
class TestGetStructureAndData(unittest.TestCase):
    ...
    @patch('climmob.views.cleanErrorLogs.getNamesEditByColums', return_value=[['name', 'description', 'type', 'constraint', 'value']])
    @patch('climmob.views.cleanErrorLogs.getQuestionsStructure', return_value=[{'name': 'name', 'id': 'id', 'vars': [{'name': 'name', 'validation': 'constraint'}]}])
    @patch('climmob.views.cleanErrorLogs.fillDataTable', return_value='filled_data')
    def test_getStructureAndData_registry(self, mock_fillDataTable, mock_getQuestionsStructure, mock_getNamesEditByColums):
        result = getStructureAndData('registry', self.view, 'userOwner', 'projectId', 'projectCod', '', 'filter')

        expected_path = os.path.join("/path/to/repository", 'userOwner', 'projectCod', 'db', 'reg', 'create.xml')
        mock_getNamesEditByColums.assert_called_once_with(expected_path)
        mock_fillDataTable.assert_called_once_with(
            self.view, 'userOwner', 'projectId', 'projectCod', 'reg',
            ['name$%*description$%*type$%*constraint$%*value$%*', 'qst162$%*Package code$%*string$%*$%*$%*'],
            expected_path, '', 'where qst162 = filter'
        )
        self.assertEqual(result, ([['name', 'description', 'type', 'constraint', 'value'], ['qst162', 'Package code', 'string', '', '']], 'filled_data'))

    @patch('climmob.views.cleanErrorLogs.getNamesEditByColums', return_value=[['name', 'description', 'type', 'constraint', 'value']])
    @patch('climmob.views.cleanErrorLogs.getQuestionsStructure', return_value=[{'name': 'name', 'id': 'id', 'vars': [{'name': 'name', 'validation': 'constraint'}]}])
    @patch('climmob.views.cleanErrorLogs.fillDataTable', return_value='filled_data')
    def test_getStructureAndData_assessment(self, mock_fillDataTable, mock_getQuestionsStructure, mock_getNamesEditByColums):
        result = getStructureAndData('assessment', self.view, 'userOwner', 'projectId', 'projectCod', 'code', 'filter')

        expected_path = os.path.join("/path/to/repository", 'userOwner', 'projectCod', 'db', 'ass', 'code', 'create.xml')
        mock_getNamesEditByColums.assert_called_once_with(expected_path)
        mock_fillDataTable.assert_called_once_with(
            self.view, 'userOwner', 'projectId', 'projectCod', 'ass',
            ['name$%*description$%*type$%*constraint$%*value$%*'],
            expected_path, 'code', 'where qst163 = filter'
        )
        expected_structure = [[['name', 'description', 'type', 'constraint', 'value', 'constraint']]]
        self.assertEqual(result, (expected_structure, 'filled_data'))

    def test_getStructureAndData_invalid_formId(self):
        with self.assertRaises(HTTPNotFound):
            getStructureAndData('invalid', self.view, 'userOwner', 'projectId', 'projectCod', 'code', 'filter')
...
class TestConvertJsonLog(unittest.TestCase):
    ...
    def test_convertJsonLog_registry(self, mock_et_parse):
        ...
        result = convertJsonLog("registry", self.view, "userOwner", "projectCod", newjson)

        expected_path = os.path.join("/path/to/repository", 'userOwner', 'projectCod', 'db', 'reg', 'manifest.xml')
    ...
    def test_convertJsonLog_assessment(self, mock_et_parse):
        ...
        result = convertJsonLog("assessment", self.view, "userOwner", "projectCod", newjson)

        expected_path = os.path.join("/path/to/repository", 'userOwner', 'projectCod', 'db', 'ass', 'test_codeid', 'manifest.xml')
        mock_et_parse.assert_called_once_with(expected_path)
...
class TestGetKeyFormManifest(unittest.TestCase):
    ...
    def test_get_key_form_manifest_registry(self, mock_et_parse):
        ...
        result = get_key_form_manifest("registry", self.view, "userOwner", "projectCod", "search_key", {})
    ...
    def test_get_key_form_manifest_assessment(self, mock_et_parse):
        ...
        result = get_key_form_manifest("assessment", self.view, "userOwner", "projectCod", "search_key", {})
    ...
    def test_get_key_form_manifest_invalid_formId(self):
        with self.assertRaises(HTTPNotFound):
            get_key_form_manifest("invalid", self.view, "userOwner", "projectCod", "search_key", {})
