...
class TestGetEnumeratorDetailsView(ViewBaseTest):
    ...
    def test_get_enumerator_details_view(self, mock_get_enumerator_data,mock_plugin_implementations):
        ...
        mock_plugin.before_returning_context.return_value = MagicMock(name="mock plugin")
        ...
        mock_get_enumerator_data.assert_called_once_with(self.view.request.matchdict["user"],self.view.request.matchdict["enumid"], self.view.request)
        mock_plugin.before_returning_context.assert_called_once_with(self.view.request, mock_enum)
