...
__all__ = [
    'generateRegistry','generateAssessmentFiles','generateRegistryPreview','generateAssessmentPreview'
]
...
def buildDatabase(cnfFile,createFile,insertFile,schema,dropSchema):
    ...
    if dropSchema:
        ...
        if not error:
            ...
            args.append("mysql")
            args.append("--defaults-file="+cnfFile)
            args.append('--execute=CREATE SCHEMA ' + schema + ' DEFAULT CHARACTER SET utf8 DEFAULT COLLATE utf8_general_ci')
            try:
                check_call(args)
            except CalledProcessError as e:
                msg = "Error dropping schema \n"
...
def createDatabase(xlsxFile,outputDir,schema,keyVar,preFix,dropSchema,request):
    ...
    return buildDatabase(request.registry.settings['mysql.cnf'],createFile, insertFile, schema,dropSchema)
...
class ODKExcelFile(object):
    ...
    def addGroup(self,name,label,appearance="field-list",inGroup=None,inRepeat=None):
    ...
    def addNodeToFile(self,rootElement):
        for element in rootElement.iterchildren():
            ...
            if element.tag == "group" or element.tag == "repeat":
                ...
                if element.tag != "root":
                    if element.get("type") != "select_one" and element.get("type") != "select_multiple":
                        self.sheet1.write(self.surveyRow, 0, element.get("type"))
                    else:
                        self.sheet1.write(self.surveyRow, 0, element.get("type") + " " + element.get("name") + "_opts")
                    ...
                    if element.get("type") == "barcode":
                        self.sheet1.write(self.surveyRow, 5, "The scanned Qr is incorrect, it is not for this project.")
                    if element.get("required") == "True":
                        self.sheet1.write(self.surveyRow, 7, element.get("label")+ " requires a value")


    def addQuestion(self,name,label,type,hint="",required=0,inGroup=None,inRepeat=None,constraint=""):
        options = {1: "text",
                   2: "decimal",
                   3: "integer",
                   4: "geopoint",
    ...
    def addOption(self,question,code,label):
        ...
        if exist:
            equestion = exist[0]
            if equestion.get("type") == "select_one" or equestion.get("type") == "select_multiple":
...
def generateODKFile(user,project,xlsxFile,formID,formLabel,groups,questions,numComb,request):
    ...
    for question in questions:
        if question.question_dtype != 8 and question.question_dtype != 9 and question.question_dtype != 10:
            if question.question_dtype != 7:
                excelFile.addQuestion(question.question_code,question.question_desc,question.question_dtype,question.question_unit,question.question_requiredvalue,"grp_"+str(question.section_id))
            else:
                #print "EL PACKAGE CODE"
                lenuser = len(user) +1
                excelFile.addQuestion(question.question_code, question.question_desc, question.question_dtype,question.question_unit, question.question_requiredvalue,"grp_" + str(question.section_id),None,"substr(${QST162}, 0, "+str(lenuser)+") = '"+user+"-' and contains(.,'-"+project+"~')")
        else:
            if question.question_dtype == 8:
                excelFile.addQuestion(question.question_code, question.question_desc, question.question_dtype,question.question_unit, question.question_requiredvalue,"grp_" + str(question.section_id))
                farmers = getRegisteredFarmers(user,project,request)
                for farmer in farmers:
                    excelFile.addOption(question.question_code, farmer["farmer_id"], farmer["farmer_name"])
            if question.question_dtype == 9:
                if numComb == 2:
                    excelFile.addQuestion("char_" + question.question_code,question.question_twoitems,5,"",question.question_requiredvalue,"grp_" + str(question.section_id))
                    for opt in range(0,numComb):
                        code = chr(65 + opt)
                        excelFile.addOption("char_" + question.question_code,str(opt+1),"Option " + code)
                if numComb == 3:
                    excelFile.addQuestion("char_" + question.question_code+"_pos",question.question_posstm,5,"",question.question_requiredvalue,"grp_"+str(question.section_id))
                    constraintGen = ". != ${"+"char_" + question.question_code+"_pos"+"}"
                    excelFile.addQuestion("char_" + question.question_code+"_neg", question.question_negstm, 5, "",question.question_requiredvalue,"grp_" + str(question.section_id),constraint=constraintGen)
                    for opt in range(0,numComb):
                        code = chr(65 + opt)
                        excelFile.addOption("char_" + question.question_code+"_pos", str(opt+1),"Option " + code)
                        excelFile.addOption("char_" + question.question_code + "_neg", str(opt+1), "Option " + code)
                if numComb >= 4:
                    constraintGen = ""
                    for opt in range(0, numComb):
                        renderedString = Environment().from_string(question.question_moreitems).render(pos=opt+1)
                        excelFile.addQuestion("char_" + question.question_code + "_stmt_" + str(opt+1), renderedString, 5, "",question.question_requiredvalue, "grp_" + str(question.section_id),constraint=constraintGen)
                        if constraintGen == "":
                            constraintGen +=". != ${"+"char_" + question.question_code + "_stmt_" + str(opt+1)+"}"
                        else:
                            constraintGen +=" and . != ${"+"char_" + question.question_code + "_stmt_" + str(opt+1)+"}"
                        for opt2 in range(0, numComb):
                            code = chr(65 + opt2)
                            excelFile.addOption("char_" + question.question_code + "_stmt_" + str(opt+1), str(opt2 + 1), "Option " + code)
            if question.question_dtype == 10:
                for opt in range(0, numComb):
                    code = chr(65 + opt)
                    renderedString = Environment().from_string(question.question_perfstmt).render(option=code)
                    excelFile.addQuestion("perf_" + question.question_code + "_" + str(opt + 1), renderedString, 5, "",question.question_requiredvalue, "grp_" + str(question.section_id))
                    excelFile.addOption("perf_" + question.question_code + "_" + str(opt + 1), "1", request.translate("Better"))
                    excelFile.addOption("perf_" + question.question_code + "_" + str(opt + 1), "2", request.translate("Worst"))

        if question.question_dtype == 5 or question.question_dtype == 6:
            print ("****generateODKFile**Query options******")
            sql = "SELECT qstoption.value_code,qstoption.value_desc,qstoption.value_isother,qstoption.value_order " \
            ...
            for value in values:
                if value.value_isother == 1:
                    other = True
                excelFile.addOption(question.question_code,value.value_code,value.value_desc)
            if other:
                excelFile.addQuestion(question.question_code+"_oth",request.translate("Other"),1,request.translate("Add the other value here"),0,"grp_" + str(question.section_id))
...
def generateRegistryPreview(user,projectid,request):
    ...
    path = os.path.join(request.registry.settings['user.repository'], *[user, projectid])
    ...
    groups = request.dbsession.query(Regsection).filter(Regsection.user_name == user).filter(
        Regsection.project_cod == projectid).order_by(Regsection.section_order).all()

    sql = "SELECT question.question_code,question.question_desc,question.question_unit,question.question_dtype,question.question_twoitems," \
    ...
    generateODKFile(user,projectid,xlsxFile, formID, label, groups, questions, prjdata["project_numcom"], request)
...
def generateRegistry(user,projectid,request):
    ...
    # Edit by Brandon
    sections_id = request.dbsession.query(Registry.section_id).distinct().filter(Registry.user_name == user).filter(Registry.project_cod == projectid)
    groups = request.dbsession.query(Regsection).filter(Regsection.user_name == user).filter(Regsection.project_cod == projectid).filter(Regsection.section_id.in_(sections_id)).order_by(Regsection.section_order).all()
    ...
    sql = "SELECT question.question_code,question.question_desc,question.question_unit,question.question_dtype,question.question_twoitems," \
          ...
          "WHERE question.question_id = registry.question_id " \
          "AND registry.user_name = '" + user + "' " \
          "AND registry.project_cod = '" + projectid + "' ORDER BY registry.question_order"
    questions = request.dbsession.execute(sql).fetchall()
    ...
    keyQuestion = request.dbsession.query(Question).filter(Question.question_regkey == 1).first()
    paths = ['db', 'reg']
    return not createDatabase(xlsxFile, os.path.join(path, *paths), user + "_" + projectid,keyQuestion.question_code,"REG",True,request)

def generateAssessmentPreview(user,projectid,assessment,request):
    assessment = request.dbsession.query(Assessment).filter(Assessment.user_name == user).filter(Assessment.project_cod == projectid).filter(Assessment.ass_cod == assessment).first()
    formID = "ASS_" + user + '_' + projectid + '_' + assessment.ass_cod + "_" + datetime.now().strftime("%Y%m%d")
    path = os.path.join(request.registry.settings['user.repository'], *[user, projectid])
    if not os.path.exists(path):
        os.makedirs(path)
    paths = ['tmp']
    ...
    groups = request.dbsession.query(Asssection).filter(Asssection.user_name == user).filter(
        Asssection.project_cod == projectid).order_by(Asssection.section_order).all()
    ...
    prjdata = getProjectData(user, projectid, request)
    label = request.translate("Data collection - ") + assessment.ass_desc + " - " + prjdata["project_name"]
    generateODKFile(user,projectid,xlsxFile, formID, label, groups, questions, prjdata["project_numcom"], request)
...
def generateAssessmentFiles(user, projectid, assessment, request):
    result = []
    keyQuestion = request.dbsession.query(Question).filter(Question.question_asskey == 1).first()
    assessments = request.dbsession.query(Assessment).filter(Assessment.user_name == user).\
        filter(Assessment.project_cod == projectid).filter(Assessment.ass_cod == assessment).all()
    for assessment in assessments:
        ...
        #Edited by Brandon
        sections_id = request.dbsession.query(AssDetail.section_id).distinct().filter(AssDetail.user_name == user).filter(AssDetail.project_cod == projectid).filter(AssDetail.ass_cod == assessment.ass_cod)
        groups = request.dbsession.query(Asssection).filter(Asssection.user_name == user).filter(Asssection.project_cod == projectid).filter(Asssection.ass_cod == assessment.ass_cod).filter(Asssection.section_id.in_(sections_id)).order_by(Asssection.section_order).all()
        #End
        ...
        sql = "SELECT question.question_code,question.question_desc,question.question_unit,question.question_dtype,question.question_twoitems," \
              ...
              "AND assdetail.user_name = '" + user + "' " \
              "AND assdetail.project_cod = '" + projectid + "' " \
              "AND assdetail.ass_cod = '" + assessment.ass_cod + "' order by assdetail.question_order"
        questions = request.dbsession.execute(sql).fetchall()
        ...
        try:
            ...
            if not os.path.exists(os.path.join(path, *paths)):
                os.makedirs(os.path.join(path, *paths))
            if createDatabase(xlsxFile, os.path.join(path, *paths), user + "_" + projectid,keyQuestion.question_code,"ASS"+assessment.ass_cod,False,request):
