...
from ...processes import isRegistryOpen,isAssessmentOpen,assessmentExists, projectExists,packageExist
...
__all__ = [
    'generateFormList','generateManifest','getFormList','getManifest',
    'getXMLForm','getMediaFile','storeSubmission','getAssessmentXMLForm',
...
def getFormList(userid,enumerator,request):
    ...
    sql = "SELECT prjenumerator.project_cod,project.project_regstatus,project.project_assstatus FROM " \
    ...
    for project in projects:
        ...
        if project.project_regstatus == 1:
            path = os.path.join(request.registry.settings['user.repository'],*[userid,project.project_cod,"odk",'reg','*.json'])
            files = glob.glob(path)
            if files:
                with io.open(files[0],encoding='utf8') as data_file:
                    data = json.load(data_file)
                    data["downloadUrl"] = request.route_url('odkxmlform',userid=userid,projectid=project.project_cod)
                    data["manifestUrl"] = request.route_url('odkmanifest', userid=userid, projectid=project.project_cod)
                prjList.append(data)

        assessments = request.dbsession.query(Assessment).filter(Assessment.user_name == userid).\
            filter(Assessment.project_cod == project.project_cod).filter(Assessment.ass_status == 1).all()
        for assessment in assessments:
            #print("yep!2")
            path = os.path.join(request.registry.settings['user.repository'],
                                *[userid, project.project_cod, "odk", 'ass', assessment.ass_cod,'*.json'])
            files = glob.glob(path)
            if files:
                with open(files[0]) as data_file:
                    data = json.load(data_file,encoding='utf8')
                    data["downloadUrl"] = request.route_url('odkxmlformass', userid=userid,projectid=project.project_cod,assessmentid=assessment.ass_cod)
                    data["manifestUrl"] = request.route_url('odkmanifestass', userid=userid,projectid=project.project_cod,assessmentid=assessment.ass_cod)
                    prjList.append(data)

    return generateFormList(prjList)

def getManifest(userid,projectid,request):
    prjdat = request.dbsession.query(Project).filter(Project.user_name == userid).filter(Project.project_cod == projectid).first()
    if prjdat.project_regstatus == 1:
        path = os.path.join(request.registry.settings['user.repository'], *[userid, projectid, "odk","reg", 'media', '*.*'])
    ...
    if files:
        ...
        for file in files:
            ...
            fileArray.append({'filename':fileName,'hash':'md5:' + md5(open(file, 'rb').read()).hexdigest(),'downloadUrl':request.route_url('odkmediafile', userid=userid, projectid=projectid, fileid=fileName)})
        return generateManifest(fileArray)
    else:
        return generateManifest([])

def getAssessmentManifest(userid,projectid,assessmentid,request):
    prjdat = request.dbsession.query(Assessment).filter(Assessment.user_name == userid). \
        filter(Assessment.project_cod == projectid).filter(Assessment.ass_cod == assessmentid).first()
    if prjdat.ass_status == 1:
        path = os.path.join(request.registry.settings['user.repository'],*[userid, projectid, "odk", "ass", assessmentid, 'media', '*.*'])
    else:
        raise HTTPNotFound()
    ...
    if files:
        ...
        for file in files:
            fileName = os.path.basename(file)
            fileArray.append({'filename':fileName,'hash':'md5:' + md5(open(file, 'rb').read()).hexdigest(),'downloadUrl':request.route_url('odkmediafileass', userid=userid, projectid=projectid,assessmentid=assessmentid,fileid=fileName)})
        return generateManifest(fileArray)
    else:
        return generateManifest([])
...
def getXMLForm(userid,projectid,request):
    prjdat = request.dbsession.query(Project).filter(Project.user_name == userid).filter(Project.project_cod == projectid).first()
    if prjdat.project_regstatus == 1:
        path = os.path.join(request.registry.settings['user.repository'],*[userid,projectid, "odk", 'reg','*.xml'])
...
def getAssessmentXMLForm(userid,projectid,assessmentid,request):
    prjdat = request.dbsession.query(Assessment).filter(Assessment.user_name == userid).\
        filter(Assessment.project_cod == projectid).filter(Assessment.ass_cod == assessmentid).first()
    if prjdat.ass_status == 1:
        path = os.path.join(request.registry.settings['user.repository'], *[userid, projectid, "odk", 'ass', assessmentid, '*.xml'])
    else:
        raise HTTPNotFound()
    files = glob.glob(path)
    if files:
...
def getMediaFile(userid,projectid,fileid,request):
    prjdat = request.dbsession.query(Project).filter(Project.user_name == userid).filter(Project.project_cod == projectid).first()
    if prjdat.project_regstatus == 1:
        path = os.path.join(request.registry.settings['user.repository'],*[userid,projectid, "odk", 'reg','media',fileid])
    else:
        raise HTTPNotFound()
...
def getAssessmentMediaFile(userid,projectid,assessmentid,fileid,request):
    prjdat = request.dbsession.query(Assessment).filter(Assessment.user_name == userid). \
        filter(Assessment.project_cod == projectid).filter(Assessment.ass_cod == assessmentid).first()
    if prjdat.ass_status == 1:
        path = os.path.join(request.registry.settings['user.repository'],*[userid, projectid, "odk", 'ass', assessmentid, 'media', fileid])
    else:
        raise HTTPNotFound()
    if os.path.isfile(path):
        content_type, content_enc = mimetypes.guess_type(path)
...
def storeError(fileid,type,userid,projectid,assessmentid,errorNumber,error,command,request):
    if errorNumber == 2:
        try:
    ...
    newError = storageErrors(fileuid=fileid,user_name=userid,project_cod=projectid,assessment_id=assessmentid,
                             submission_type=type,command_executed=command,error_cod=sqlErrorNumber,
                             error_des=sqlErrorDesc,error_table=sqlTable,
...
def storeJSONInMySQL(type,userid,projectid,assessmentid,JSONFile,request):
    ...
    if type == "REG":
        manifestFile = os.path.join(request.registry.settings['user.repository'],*[userid, projectid, "db", 'reg', 'manifest.xml'])
        jsFile = os.path.join(request.registry.settings['user.repository'],*[userid, projectid, "db", 'reg', 'custom.js'])
    else:
        manifestFile = os.path.join(request.registry.settings['user.repository'],*[userid, projectid, "db", 'ass', assessmentid, 'manifest.xml'])
        jsFile=""
    ...
    if p.returncode != 0:
        ...
        print (" ".join(args))
        print ("*********************666")
        storeError(fileuid,type,userid,projectid,assessmentid,p.returncode,stdout," ".join(args),request)
    return True

def convertXMLToJSON(userid, userEnum,XMLFile,JSONFile,projectID,submissionType,assessmentID,request):
    ...
    if files:
        ...
        try:
            ...
            with open(JSONFile, "w") as outfile:
                ...
                data["_submitted_date"] = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
                jsonString = json.dumps(data, indent=4, ensure_ascii=False)
                outfile.write(jsonString)
            #Now we store the data in MySQL
            storeJSONInMySQL(submissionType,userid,projectID,assessmentID,JSONFile,request)
...
def storeSubmission(userid,userEnum,request):
    ...
    for key in request.POST.keys():
        ...
        if filename.upper().find('.XML') >= 0:
            ...
            input_file.seek(0)
            acceptSubmission,error,projectID,submissionType,assessmentID = checkSubmission(userid,input_file,request)
    if acceptSubmission:
        iniqueID = uuid4()
        if submissionType == "REG":
            path = os.path.join(request.registry.settings['user.repository'],*[userid, projectID, "data", 'reg', 'xml',str(iniqueID)])
            if not os.path.exists(path):
                os.makedirs(path)
                os.makedirs(os.path.join(request.registry.settings['user.repository'],*[userid, projectID, "data", 'reg', 'json',str(iniqueID)]))

        else:
            path = os.path.join(request.registry.settings['user.repository'],*[userid, projectID, "data", 'ass', assessmentID, 'xml', str(iniqueID)])
            if not os.path.exists(path):
                os.makedirs(path)
                os.makedirs(os.path.join(request.registry.settings['user.repository'],*[userid, projectID, "data", 'ass', assessmentID, 'json', str(iniqueID)]))
        ...
        if XMLFile != "":
            ...
            convertXMLToJSON(userid,userEnum, XMLFile,JSONFile,projectID,submissionType,assessmentID,request)
            return True, 201
        else:
            return False,500
    else:
