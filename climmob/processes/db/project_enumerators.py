...
__all__ = [
    'getProjectEnumerators','addEnumeratorToProject','removeEnumeratorFromProject','getUsableEnumerators','seeProgress','getAssessmenstByProject'
]
...
def getProjectEnumerators(user,project,request):
    ...
    return result
    """
    result = request.dbsession.query(Enumerator).filter(PrjEnumerator.user_name == Enumerator.user_name).filter(PrjEnumerator.enum_id == Enumerator.enum_id).filter(PrjEnumerator.user_name == user).filter(PrjEnumerator.project_cod == project).all()
...
def seeProgress(user,project,request):
    all = {}
    try:
        sql = "SELECT _submitted_by as user_name, count(_submitted_by) as count FROM "+user+"_"+project+".REG_geninfo group by _submitted_by"
    ...
    try:
        ...
        for assessment in assessments:
            ...
            sql = "SELECT _submitted_by as user_name, count(_submitted_by) as count FROM " + user + "_" + project + ".ASS"+assessment["ass_cod"]+"_geninfo group by _submitted_by"
...
def getAssessmenstByProject(user,project,request):
    res = request.dbsession.query(Assessment).filter(Assessment.user_name == user).filter(Assessment.project_cod == project).filter(or_(Assessment.ass_status == 1, Assessment.ass_status == 2)).order_by(Assessment.ass_days.asc()).all()
    res = mapFromSchema(res)
    return res

def addEnumeratorToProject(user,project,enumerator,request):
    newEnumerator = PrjEnumerator(user_name=user,project_cod=project,enum_user=user,enum_id=enumerator)
    try:
...
def removeEnumeratorFromProject(user,project,enumerator,request):
    try:
        request.dbsession.query(PrjEnumerator).filter(PrjEnumerator.user_name == user).filter(PrjEnumerator.project_cod==project).filter(PrjEnumerator.enum_id==enumerator).delete()
        return True,""
    except Exception as e:
        print (str(e))
        return False, e
...
def getUsableEnumerators(user,project,request):
    sql = "SELECT enum_id,enum_name FROM enumerator " \
