...
__all__ = [
    'getProjectEncrypted','getRegistryInformation','AssessmentsInformation'
]
...
def getProjectEncrypted(request,encrypted):
    ...
    result = request.dbsession.query(Project).filter(func.md5(func.concat(Project.user_name,'#$%&',Project.project_cod)) == encrypted).first()
    if result:
...
def getRegistryInformation(request,info):
    ...
    submissions = 0
    sql = "SELECT COUNT(*) as total FROM " + info["user_name"] + "_" + info["project_cod"] + '.REG_geninfo'
    try:
...
def AssessmentsInformation(request, info, registered):
    assessmentArray = []
    assessments = request.dbsession.query(Assessment).filter(Assessment.user_name == info["user_name"]).filter(Assessment.project_cod == info["project_cod"]).all()
    for assessment in assessments:
        submissions = 0

        sql = "SELECT COUNT(*) as total FROM " + info["user_name"] + "_" + info["project_cod"] + ".ASS" + assessment.ass_cod + '_geninfo'
        try:
            res = request.repsession.execute(sql).fetchone()
            submissions = res["total"]
        except:
