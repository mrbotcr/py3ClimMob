...
__all__ = [
    'userExists','emailExists','otherUserHasEmail','projectExists','projectTechnologyExists',
    'projectInDatabase','projectRegStatus'
]
...
def otherUserHasEmail(user,email,request):
    res = False
    result = request.dbsession.query(User).filter(User.user_name != user).filter_by(user_email = email).first()
    if not result is None:
        res = True
    return  res
...
def projectTechnologyExists(user, projectid, technologyid,request):
    result = request.dbsession.query(Prjtech).filter(Prjtech.tech_id == technologyid).filter(Prjtech.project_cod == projectid).filter(Prjtech.user_name == user).first()
    if not result:
        return False
    else:
        return True