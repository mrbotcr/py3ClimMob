...
def addToLog(log_user, log_type, log_message,log_datetime, request):
    newLog = Activitylog(log_user=log_user, log_type=log_type, log_message=log_message, log_datetime=log_datetime)
    try:
...
def getLastActivityLogByUser(log_user,request):
    result = (
        request.dbsession.query(Activitylog).filter(Activitylog.log_user == log_user).order_by(Activitylog.log_datetime.desc()).first()
    )
