...
__all__ = [
    'getCountryList','getSectorList','getUserLog','getUserStats','getUserPassword'
]
...
def getUserLog(user,request,limit = True):
    sql = "SELECT DATE_FORMAT(DATE(log_datetime), '%W %D of %M, %Y') as log_date,TIME(log_datetime) as log_time,log_type,log_message,log_datetime as date1,log_datetime as date2 FROM activitylog WHERE log_user = '" + user + "' ORDER BY date1 DESC,date2 ASC,log_id desc"
    if limit:
        sql = sql + " LIMIT 20"
    ...
    for activity in activities:
        ...
        items.append({"date":activity[0],"time":activity[1],"type":desType,"message":activity[3],"alt":alt,"icon":icon,"color":color})
    return items
...
def getUserStats(user,request):
    ...
    sql = "SELECT max(project_creationdate) FROM project WHERE user_name = '" + user + "'"
    ...
    return {'totprojects': projects[0],'lastproject':lastProject,'totregistry':0,'totassessment':0}
