        ...
        if etable is not None:
            for child in etable.iterchildren():
                if child.tag == "field":
                    ...
                    if field["isMultiSelect"] == "false" or field["isMultiSelect"] is None:
                        field["rtable"] = child.get("rtable")
                        field["rfield"] = child.get("rfield")
                    else:
                        field["rtable"],field["rfield"] = getMiltiSelectLookUpTable(XMLFile,field["multiSelectTable"])
                    fields.append(field)
    return fields
    ...
    if elkptables is not None:
        ...
        for table in etables:
            ...
            if atable["fields"]:
                ...
                sql = "SELECT " + ",".join(fieldArray) + " FROM " + user + "_" + project + "." + atable["name"]
...
def getPackageData(user,project,request):
    data = request.dbsession.query(Question).filter(Question.question_regkey == 1).first()
    qstPackage = data.question_code
    data = request.dbsession.query(Question).filter(Question.question_fname == 1).first()
    qstFarmer = data.question_code

    sql = "SELECT project.project_cod,project.project_numcom,count(prjtech.tech_id) as ttech FROM " \
    ...
    sql = "SELECT * FROM ((" \
          ...
          "FROM pkgcomb,package,prjcombination,prjcombdet,prjalias,techalias, technology,user,project," + user + "_" + project + ".REG_geninfo " \
          ...
          "AND pkgcomb.package_id = " + user + "_" + project + ".REG_geninfo." + qstPackage + " " \
          ...
          "pkgcomb.comb_order," + user + "_" + project + ".REG_geninfo." + qstFarmer + " " \
          "FROM pkgcomb,package,prjcombination,prjcombdet,prjalias, technology,user,project," + user + "_" + project + ".REG_geninfo " \
          "WHERE pkgcomb.user_name = user.user_name " \
          "AND pkgcomb.project_cod = project.project_cod " \
          "AND pkgcomb.user_name = package.user_name " \
          "AND pkgcomb.project_cod = package.project_cod " \
    ...
    for pkg in pkgdetails:
        if pkgcode != pkg.package_id:
            ...
            aPackage["comps"][pkg.comb_order - 1]["technologies"].append({"tech_name": pkg.tech_name,"alias_name":pkg.alias_name})
            packages.append(aPackage)
        else:
            try:
                packages[len(packages) - 1]["comps"][pkg.comb_order-1]["comb_order"] =  pkg.comb_order
                packages[len(packages) - 1]["comps"][pkg.comb_order-1]["technologies"].append({"tech_name": pkg.tech_name,"alias_name":pkg.alias_name})
            except:
                packages[len(packages) - 1]["comps"][pkg.comb_order - 1]["technologies"] = []
                packages[len(packages) - 1]["comps"][pkg.comb_order - 1]["technologies"].append({"tech_name": pkg.tech_name,"alias_name":pkg.alias_name})
...
def getData(user,project,registry,assessments,request):
    ...
    data = request.dbsession.query(Question).filter(Question.question_asskey == 1).first()
    assessmentKey = data.question_code

    fields = []
    for field in registry["fields"]:
        fields.append(user + "_" + project + ".REG_geninfo." + field["name"] + " AS " + "REG_" + field["name"])
    for assessment in assessments:
        for field in assessment["fields"]:
            fields.append(user + "_" + project + ".ASS" + assessment["code"] + "_geninfo." + field["name"] + " AS " + "ASS" + assessment["code"] + "_" + field["name"])

    sql = "SELECT " + ",".join(fields) + " FROM " + user + "_" + project + ".REG_geninfo "
    for assessment in assessments:
        sql = sql + " LEFT JOIN " + user + "_" + project + ".ASS" + assessment["code"] + "_geninfo ON "
        sql = sql + user + "_" + project + ".REG_geninfo." + registryKey + " = " + user + "_" + project + ".ASS" + assessment["code"] + "_geninfo." + assessmentKey
...
def getImportantFields(user,project,request):
    ...
    prjData = request.dbsession.query(Project).filter(Project.user_name == user).filter(
        Project.project_cod == project).first()

    data = request.dbsession.query(Question).filter(Question.question_regkey == 1).first()
    registryKey = data.question_code
    result.append({'type':"PackageID",'field': "REG_" + registryKey,'desc':"Package ID"})
    data = request.dbsession.query(Question).filter(Question.question_fname == 1).first()
    registryKey = data.question_code
    result.append({'type':"FarmerName",'field': "REG_" + registryKey,'desc':"Name of registered farmer"})

    sql = "SELECT assdetail.ass_cod,question_code,question_overall,question_overallperf " \
    ...
    for question in data:
        if question.question_overall == 1:
            if prjData.project_numcom == 2:
                result.append({'type': "OverallChar",
                               'field': "ASS" + question.ass_cod + "_char_" + question.question_code,'desc': "Over all characteristic positive"})
            if prjData.project_numcom == 3:
                result.append({'type':"OverallCharPos",'field': "ASS" + question.ass_cod + "_char_" + question.question_code + "_pos",'desc':"Over all characteristic positive"})
                result.append({'type':"OverallCharNeg", 'field': "ASS" + question.ass_cod + "_char_" + question.question_code + "_neg",'desc': "Over all characteristic negative"})
            if prjData.project_numcom >= 4:
                for comp in range(0, prjData.project_numcom):
                    result.append({'type': "OverallCharPosition" + str(comp+1),
                                   'field': "ASS" + question.ass_cod + "_char)" + question.question_code + "_stmt_" + str(comp+1),'desc': "Over all characteristic for position " + str(comp+1)})
        if question.question_overallperf == 1:
            for comp in range(0,prjData.project_numcom):
                result.append(
                    {'type': "OverallPerf" + str(comp+1), 'field': "ASS" + question.ass_cod + "_perf_" + question.question_code + "_" + str(comp+1),'desc': "Over all performance of " + chr(65+comp) + " against current" })
    return result
...
def getSpecialFields(registry,assessments):
    ...
    for field in registry["fields"]:
        if field["name"][:5] == "char_":
            result.append({'type':'Characteristic','name': 'REG_' + field["name"],'desc': field["desc"]})
        if field["name"][:5] == "perf_":
            result.append({'type': 'Performance', 'name': 'REG_' + field["name"],'desc': field["desc"]})
    for assessment in assessments:
        for field in assessment["fields"]:
            if field["name"][:5] == "char_":
                result.append({'type': 'Characteristic', 'name': 'ASS' + assessment["code"] + '_' + field["name"], 'desc': field["desc"]})
            if field["name"][:5] == "perf_":
                result.append({'type': 'Performance', 'name': 'ASS' + assessment["code"] + '_' + field["name"],
                               'desc': field["desc"]})
    return result

def getJSONResult(user,project,request):
    data = {}
    res = request.dbsession.query(Project).filter(Project.user_name == user).filter(
        Project.project_cod == project).first()
    if res is not None:
        if res.project_regstatus == 1 or res.project_regstatus == 2:
            ...
            registryXML = os.path.join(request.registry.settings['user.repository'],
                                       *[user, project, 'db', 'reg', 'create.xml'])
            if os.path.exists(registryXML):
                data["registry"] = {'lkptables':getLookups(registryXML,user,project,request),'fields':getFields(registryXML,'REG_geninfo')}
            ...
            for assessment in assessments:
                if assessment.ass_status == 1 or assessment.ass_status == 2:
                    ...
                    assessmentXML = os.path.join(request.registry.settings['user.repository'],
                                           *[user, project, 'db', 'ass', assessment.ass_cod, 'create.xml'])
                    if os.path.exists(registryXML):
                        data["assessments"].append({"code":assessment.ass_cod,"desc":assessment.ass_desc,'lkptables':getLookups(assessmentXML,user,project,request),
                                                    'fields':getFields(assessmentXML,'ASS' + assessment.ass_cod + '_geninfo')})
            #Get the package information but only for registered farmers
            data["packages"] = getPackageData(user,project,request)
            if haveAssessments:
                data["specialfields"] = getSpecialFields(data["registry"], data["assessments"])
                data["data"] = getData(user,project,data["registry"],data["assessments"],request)
