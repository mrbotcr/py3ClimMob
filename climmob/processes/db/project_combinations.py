...
__all__ = [
    'createCombinations','getCombinations','setCombinationStatus','projectHasCombinations','getCombinationStatus','projectCreateCombinations','projectCreatePackages','getTech'
]


def projectHasCombinations(user,project,request):
    data = request.dbsession.query(Prjcombination).filter(Prjcombination.user_name == user).\
        filter(Prjcombination.project_cod == project).first()
    if data is not None:
        return True
    else:
        return False
...
def getCombinationStatus(user,project,comb_code,request):
    result = request.dbsession.query(Prjcombination).filter(Prjcombination.user_name == user).filter(Prjcombination.project_cod == project).filter(Prjcombination.comb_code == comb_code).first()
...
def setCombinationStatus(user,project,id,status,request):
    request.dbsession.query(Prjcombination).filter(Prjcombination.user_name == user).\
        filter(Prjcombination.project_cod == project).filter(Prjcombination.comb_code == id).update({'comb_usable': status})
    request.dbsession.query(Project).filter(Project.user_name == user).filter(
        Project.project_cod == project).update({'project_createpkgs': 1})


def getTech(user,project,request):
    sql = "SELECT prjcombdet.tech_id,technology.tech_name FROM " \
...
def getCombinations(user,project,request):
    ...
    sql = "select max(comb_code) as maxcomb FROM " \
...
def projectCreateCombinations(user,project,request):
    prjData = request.dbsession.query(Project).filter(Project.user_name == user).filter(Project.project_cod == project).first()
...
def createCombinations(user,project,request):
    try:
        ...
        if prjData.project_createcomb == 1:
            sql = "SELECT DISTINCT prjalias.tech_id,technology.tech_name FROM prjalias,technology " \
            ...
            for tech in techs:
                ...
                alias_array = []
                sql = "SELECT prjalias.alias_id,prjalias.alias_used,techalias.alias_name FROM prjalias,techalias " \
                ...
                for alias in alises:
                    alias_array.append({'tech_id':str(tech_id),'tech_name':tech.tech_name,'alias_id':str(alias.alias_id),'alias_used':str(alias.alias_used),'alias_name':alias.alias_name})
                if len(alias_array) > 0:
                    tech_array.append(list(alias_array))
                    tech_used.append({'tech_id': str(tech_id), 'tech_name': tech.tech_name})
            ...
            if len(tech_array) > 1:
                ...
                request.dbsession.query(Prjcombination).filter(Prjcombination.user_name == user).filter(Prjcombination.project_cod == project).delete()
                combNumber = 1
                for combination in combinations:
                    newCombination = Prjcombination(user_name=user,project_cod=project,comb_code=combNumber,comb_usable=1)
                    request.dbsession.add(newCombination)
                    aliasNumber = 1
                    for aAlias in combination:
                        newAliasUsed = Prjcombdet(prjcomb_user=user,prjcomb_project=project,comb_code=combNumber,user_name=user,
                                                  project_cod=project,tech_id=aAlias["tech_id"],alias_id=aAlias["alias_id"],
                ...
                request.dbsession.query(Project).filter(Project.user_name == user).filter(
                    Project.project_cod == project).update({'project_createcomb':0})
                ...
                #print tech_array
                #print "*******************11"
                request.dbsession.query(Prjcombination).filter(Prjcombination.user_name == user).filter(
                    Prjcombination.project_cod == project).delete()
                aliasNumber = 1
                for aAlias in tech_array:
                    newCombination = Prjcombination(user_name=user, project_cod=project, comb_code=aliasNumber, comb_usable=1)
                    request.dbsession.add(newCombination)
                    newAliasUsed = Prjcombdet(prjcomb_user=user, prjcomb_project=project, comb_code=aliasNumber, user_name=user,
                                              project_cod=project, tech_id=aAlias["tech_id"], alias_id=aAlias["alias_id"],
                                              alias_order=aliasNumber)
                    request.dbsession.add(newAliasUsed)
                    aliasNumber += 1
