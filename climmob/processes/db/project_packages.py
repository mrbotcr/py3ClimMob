...
__all__ = [
    'create_packages_with_r', 'getPackages', 'projectHasPackages'
]
...
def projectHasPackages(user,project,request):
    data = request.dbsession.query(Package).filter(Package.user_name == user).\
        filter(Package.project_cod == project).first()
    if data is not None:
        return True
    else:
        return False
    ...
    for pkg in pkgdetails:
        if pkgcode != pkg.package_id:
            ...
            aPackage["combs"][pkg.comb_order - 1]["technologies"].append(
                {"tech_id":pkg.tech_id,"tech_name": pkg.tech_name, "alias_name": pkg.alias_name})
            packages.append(aPackage)
        else:
            try:
                packages[len(packages) - 1]["combs"][pkg.comb_order - 1]["comb_order"] = pkg.comb_order
                packages[len(packages) - 1]["combs"][pkg.comb_order - 1]["technologies"].append(
                    {"tech_id":pkg.tech_id,"tech_name": pkg.tech_name, "alias_name": pkg.alias_name})
            except:
                packages[len(packages) - 1]["combs"][pkg.comb_order - 1]["technologies"] = []
                packages[len(packages) - 1]["combs"][pkg.comb_order - 1]["technologies"].append(
                    {"tech_id":pkg.tech_id,"tech_name": pkg.tech_name, "alias_name": pkg.alias_name})
...
def create_packages_with_r(user, project, request):
    path = os.path.join(request.registry.settings['user.repository'], *[user, project, 'r'])
    if not os.path.exists(path):
        os.makedirs(path)
    rfile = os.path.join(request.registry.settings['user.repository'], *[user, project, 'r', 'comb.txt'])
    rout = os.path.join(request.registry.settings['user.repository'], *[user, project, 'r', 'comb_2.txt'])
    ...
    if prjData.project_createpkgs == 1:
        combData = request.dbsession.query(Prjcombination).filter(Prjcombination.user_name == user).filter(
            Prjcombination.project_cod == project).filter(Prjcombination.comb_usable == 1).all()
        ...
        if combinations:
            ...
            try:
                ...
                request.dbsession.query(Package).filter(Package.user_name == user).filter(
                    Package.project_cod == project).delete()

                with open(rout) as fp:
                    lines = fp.readlines()
                    pkgid = 1
                    for line in lines:
                        newPackage = Package(user_name=user, project_cod=project, package_id=pkgid,
                        ...
                        combid = 1
                        for comb in combs:
                            newPkgcomb = Pkgcomb(user_name=user, project_cod=project, package_id=pkgid, comb_user=user,
                                                 comb_project=project, comb_code=int(comb), comb_order=combid)
...
def createPackages(user,project,request):
    ...
    if prjData.project_createpkgs == 1:
        ...
        if combinations:
            ...
            for grp in groups:
                newPackage = Package(user_name=user,project_cod=project,package_id=pkgid,package_code="Package #" + str(pkgid))
                request.dbsession.add(newPackage)
                combid = 1
                for comb in grp:
                    newPkgcomb = Pkgcomb(user_name=user, project_cod=project, package_id=pkgid, comb_user=user,
