...
__all__ = [
    'addUser','updateProfile','changeUserPassword','getUserCount','getUserInfo'
]
...
def changeUserPassword(user,password,request):
    try:
        request.dbsession.query(User).filter_by(user_name = user).update({"user_password": encodeData(request, password)})
        return True
    except:
        return False
