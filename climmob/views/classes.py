...
class odkView(object):
    ...
    def authorize(self,correctPassword):
        ...
        if self.authHeader["qop"] == "auth":
            ha1 = md5(
                (
                        self.user + ":" + self.realm + ":" + correctPassword.decode()
                ).encode()
            )
            ha2 = md5(
...
class privateView(object):
    ...
    def __call__(self):
        ...
        self.classResult["userProjects"] = getUserProjects(self.user.login, self.request)
...
class apiView(object):
    ...
    def __call__(self):
        ...
        try:
            ...
            if (self.user == None):
                response = Response(status=401, body=self.request.translate("This Apikey does not exist or is inactive."))
                return response
            ...
            if self.request.method == "POST":
                try:
                    ...
                    response = Response(status=401, body=self.request.translate("Body non-existent"))
                    return response

        except:
            response = Response(status=401, body=self.request.translate("Apikey non-existent"))
            return response
