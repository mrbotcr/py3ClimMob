from ..plugins.utilities import climmobPublicView,climmobPrivateView, getProductDirectory,getProducts
...
def getDataProduct(user, project, request):
    ...
    sql = "select edited.celery_taskid,edited.user_name,edited.project_cod,edited.product_id, edited.datetime_added, edited.output_id,edited.state, edited.output_mimetype " \
            "from " \
          "(" \
          ...
          ") " \
          "as edited " \
          "where edited.datetime_added = (select max(f.datetime_added) from products f where f.output_id = edited.output_id) and edited.user_name='"+user+"' and edited.project_cod='"+project+"'"
...
class productsView(climmobPrivateView):
    def processView(self):
        ...
        if activeProjectData:
            products = getDataProduct(self.user.login,activeProjectData["project_cod"],self.request)
        ...
        return {'activeUser': self.user, 'activeProject': activeProjectData,
                'hasActiveProject': hasActiveProject, 'Products':products}
...
class downloadJsonView(climmobPrivateView):
    def processView(self):
        ...
        activeProjectData = getActiveProject(self.user.login, self.request)
        dataworking = getProductData(self.user.login, activeProjectData["project_cod"], celery_taskid, self.request)
        ...
        if product_found(product_id):
            ...
            path = getProductDirectory(self.request, self.user.login, activeProjectData["project_cod"], product_id)
