    ...
    def processView(self):
        ...
        if (self.request.method == 'POST'):
            if 'btn_createAnalysis' in self.request.POST:
                ...
                data = getQuestionsByType(self.user.login,activeProjectData["project_cod"],self.request)
                dict = {}
                ...
                for _section in data:
                    ...
                    for _info in data[_section]:
                        if _info["id"]  in use:
                            dict[_section].append(data[_section][data[_section].index(_info)])

                infosheet = dataworking["txt_infosheets"].upper()
                #print json.dumps(dict)
                locale = self.request.locale_name
                create_analysis(locale,self.user.login, activeProjectData["project_cod"], dict, getJSONResult(self.user.login,activeProjectData["project_cod"],self.request), infosheet, self.request)
                self.returnRawViewResult = True
                return HTTPFound(location=self.request.route_url('productList'))


        return {'activeUser': self.user, "dataForAnalysis":getQuestionsByType(self.user.login,activeProjectData["project_cod"],self.request), "correct": False}