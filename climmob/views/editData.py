...
class downloadDataView(privateView):
    def processView(self):
        ...
        if not projectExists(self.user.login, proId, self.request):
            ...
            # Add by Brandon
            path = os.path.join(self.request.registry.settings['user.repository'], *[self.user.login, proId])
            if code == "":
        ...
        for column in columns:
            try:
                selected_contacts.append(column[0]+"$%*"+column[1]+"$%*"+column[2]+"$%*"+column[3]+"$%*"+column[4]+"$%*"+column[5])
            except:
    ...
    def processView(self):
        ...
        if not projectExists(self.user.login, proId, self.request):
            ...
            if self.request.method == "GET":
                ...
                return {'activeUser': self.user,'error_summary':error_summary, "correct":False}
            else:
                ...
                correct = False
                if _json["projectMeta"][0]["code"] == code and _json["projectMeta"][0]["project_code"] == proId and _json["projectMeta"][0]["form"] == formId and _json["projectMeta"][0]["user"] == self.user.login:
                    json_data = self.request.POST.getall("dataNeccessaryUpload")
                    if json_data[0] != '':
                        output = update_edited_data(self, proId, formId, json_data, path, code)
                        if output == 0:
                            error_summary = {'error': self._("You have information with errors.")}
                        else:
                            correct = True
                            print ("Termina la edicion de datos.")
                else:
                    error_summary = {'error': self._("This data does not belong to this form.")}

                return {'activeUser': self.user, 'error_summary': error_summary, "correct":correct}
        ...
        ####
            ...
            if 'btn_EditData' in self.request.POST:
                ...
                if len(selected_contacts) == 0:  # if non selected columns in check options
                    ...
                    dataworking['fill'] = fillDataTable(self, proId, formId, selected_contacts, path, code)
                    dataworking['error'] = ''
                ...
                if 'json_data' in self.request.POST:
                    ...
                    #u.getCSSResource('sweet').need()
                    if json_data[0] != '':
                        dataworking['msg_flag'] = update_edited_data(self, proId, formId, json_data, path, code)
            ...
            return {'dataworking': dataworking, 'activeUser': self.user,'formId':formId, 'newStructure': newStructure}
