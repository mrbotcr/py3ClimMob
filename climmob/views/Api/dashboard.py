    ...
    def processView(self):
        ...
        if self.request.method == "GET":
            ...
            if sorted(obligatory) == sorted(dataworking.keys()):
                ...
                if exitsproject:
                    projectDetails = getProjectData(self.user.login,dataworking['project_cod'],self.request)
                    progress, pcompleted = getProjectProgress(self.user.login, dataworking['project_cod'], self.request)
                    response = Response(status=200, body=json.dumps({'project': projectDetails,'progress': progress, 'pcompleted': pcompleted}, default=myconverter))
                    return response
                else:
