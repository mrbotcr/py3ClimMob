...
from ..processes import addUser,addToLog,getCountryList,getSectorList,getUserCount,getProjectCount
...
class home_view(publicView):
    def processView(self):
        ...
        return {'numUsers': getUserCount(self.request),'numProjs':getProjectCount(self.request)}
        ...
        if 'submit' in self.request.POST:
            ...
            if email is not None:
                ...
                if user is not None:
                    ...
                    message = message + self._("You requested ClimMob Version 3 to send you your password.\n\n")
                    ...
                    error_summary["email"] = self._("Cannot find an user with such email address")
            else:
...
class register_view(publicView):
    def processView(self):
                    ...
                    if user is not None:
                        ...
                        if reg.match(data["user_name"]):
                            if user.check_password(data["user_password"],self.request):
                                addToLog(user.login, 'PRF', self._("Welcome to Climmob"),self.request)
                                headers = remember(self.request, data["user_name"])
                                self.returnRawViewResult = True
                                return HTTPFound(location=self.request.route_url('dashboard'), headers=headers)
                            else:
                                error_summary["createError"] = self._("Password does not match {}".format(data["user_password"]))
                        else:
                            error_summary["createError"] = self._("The account name cannot have any characters.")
                    else:
        ...
        return {'data': data, 'error_summary': error_summary,'countries': getCountryList(self.request), 'sectors': getSectorList(self.request)}