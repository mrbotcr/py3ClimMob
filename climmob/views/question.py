...
from ..processes import addQuestion,addOptionToQuestion,updateQuestion,\
    ...
    getOptionData,updateOption,questionExists, categoryExists, addCategory,getCategories,updateCategory, deleteCategory
...
class modifyQuestion_view(privateView):
    def processView(self):
        ...
        if self.request.method == 'POST':
            ...
            if formdata['question_desc'] != "" and formdata['question_dtype'] != "":
                ...
                if not updated:
                    ...
                    self.request.session.flash(self._('The question was successfully modified'))
                    redirect = True
            else:
                if formdata['question_alwaysinreg'] == 1:
                    formdata['question_alwaysinreg'] = 'on'
        ...
        return {'activeUser': self.user, 'error_summary': error_summary, 'formdata': self.decodeDict(formdata), 'redirect': redirect, 'Categories': getCategories(self.user.login,self.request)}
...
class deleteQuestionValue_view(privateView):
    def processView(self):
        ...
        return {'activeUser': self.user, 'qdata': qdata, 'qid': qid, 'formdata': formdata,
...
class modifyQuestionValue_view(privateView):
    def processView(self):
        ...
        return {'activeUser': self.user, 'qdata': qdata, 'qid': qid, 'formdata': self.decodeDict(formdata),
                'error_summary': error_summary}
...
class addQuestionValue_view(privateView):
    def processView(self):
        ...
        if self.request.method == 'POST':
            ...
            formdata["value_code"] = re.sub('[^A-Za-z0-9\-]+', '', formdata["value_code"])
        ...
        return {'activeUser': self.user, 'qdata': qdata,'qid':qid,'formdata':self.decodeDict(formdata),'error_summary':error_summary}
    ...
    def processView(self):
        ...
        if qdata:
            if qdata["question_dtype"] == 5 or qdata["question_dtype"] == 6:
                ...
                #self.needJS("sweet")
                #self.needJS("delete")
                return {'activeUser': self.user,'qdata':qdata,'qoptions':qoptions,'qid':qid,'editable':editable}
            else:
                raise HTTPNotFound()
        else:
            raise HTTPNotFound()
    ...
    def processView(self):
        ...
        if qdata["question_dtype"] == 10:
            ...
            if self.request.method == 'GET':
                ...
                return {'activeUser': self.user, 'qdata': qdata,'qid':qid,'error_summary':error_summary,'redirect':redirect,'withtoastr':withtoastr,'editable':editable}
            if self.request.method == 'POST':
                if not editable:
                    raise HTTPNotFound()
                formdata = self.getPostDict()
                ...
                if perstmt.find('{{option}}') >= 0:
                    ...
                    if modified:
                        if withtoastr:
                            self.request.session.flash(self._('The question was successfully added'))
                        else:
                            self.request.session.flash(self._('You successfully updated the performance statement.'))
                        redirect = True
                    else:
                        error_summary = {'dberror': msg}
                else:
                    error_summary[
                        "stmterror"] = self._("The performance statement must have the wildcard {{option}}")
                return {'activeUser': self.user, 'qdata': self.decodeDict(qdata), 'qid': qid, 'error_summary': error_summary,'redirect':redirect,'withtoastr':withtoastr,'editable':editable}
        else:
            raise HTTPNotFound()
            ...
            if self.request.method == 'POST':
                ...
                if modified:
                    if withtoastr:
                        ...
                        self.request.session.flash(self._('You successfully updated the characteristic.'))
                    redirect = True
                else:
                    error_summary = {'dberror': msg}
...
class newQuestion_view(privateView):
    def processView(self):
        ...
        if self.request.method == 'POST':
            ...
            if formdata['question_code'] != "" and formdata['question_desc'] != "" and formdata['question_dtype'] != "":
                if not questionExists(self.user.login,formdata['question_code'],self.request):
                    ...
                    if not add:
                        ...
                        if formdata['question_dtype'] == 5 or formdata['question_dtype'] == 6 or formdata['question_dtype'] == 9 or formdata['question_dtype'] == 10:
                            if formdata['question_dtype'] == 5 or formdata['question_dtype'] == 6:
                                self.request.session.flash(self._('The question was successfully added. Add new values now'))
                                self.returnRawViewResult = True
                                return HTTPFound(location=self.request.route_url('questionvalues', qid=idorerror))
                            else:
                                if formdata['question_dtype'] == 9:
                                    self.request.session.flash(self._('The question was successfully added. Configure the characteristic now'))
                                    self.returnRawViewResult = True
                                    return HTTPFound(location=self.request.route_url('questioncharacteristics', qid=idorerror,_query={'withtoastr':True}))
                                else:
                                    self.request.session.flash(self._('The question was successfully added. Configure the performance statement now'))
                                    self.returnRawViewResult = True
                                    return HTTPFound(location=self.request.route_url('questionperformance', qid=idorerror,_query={'withtoastr': True}))
                        else:
                            self.request.session.flash(self._('The question was successfully added'))
                            redirect = True
                else:
                    ...
                    error_summary = {'same': self._("There is another question with the same code.")}
...
class qlibrary_view(privateView):
    def processView(self):
        ...
        return {'activeUser':self.user,'UserQuestion':UserQuestion(user_name,self.request), "showing": user_name,
    ...
    def processView(self):
        ...
        if self.request.method == 'POST':
            ...
            if postdata["action"] == "new":
                ...
                if not categoryExists(self.user.login, postdata["qstgroups_name"], self.request):
                    added, message = addCategory(self.user.login, postdata, self.request)
                    if not added:
                        self.returnRawViewResult = True
                        return {'result': "error", 'error': self._("Could not create category")}
                    else:
                        self.returnRawViewResult = True
                        return {'result': "success", "success": self._('The category was created successfully')}
                else:
                    self.returnRawViewResult = True
                    return {'result': "error", 'error': self._("This category name already exists.")}
            else:
                if postdata["action"] == "update":
                    if not categoryExists(self.user.login, postdata["qstgroups_name"], self.request):
                        update, message = updateCategory(self.user.login, postdata, self.request)
                        if not update:
                            self.returnRawViewResult = True
                            return {'result': "error", 'error': self._("Could not update the category")}
                        else:
                            self.returnRawViewResult = True
                            return {'result': "success", "success": self._('The category was updated successfully')}
                    else:
                        self.returnRawViewResult = True
                        return {'result': "error", 'error': self._("This category name already exists.")}
                else:
                    if postdata["action"] == "delete":
                        deleted, message = deleteCategory(self.user.login, postdata["qstgroups_id"], self.request)
                        if not deleted:
                            self.returnRawViewResult = True
                            return {'result': "error", 'error': self._("Could not delete the category")}
                        else:
                            self.returnRawViewResult = True
                            return {'result': "success", "success": self._('The category was deleted successfully')}
        return {}