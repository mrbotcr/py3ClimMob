...
def get_FieldsByType(types, db, self, file, code):
    return [item[0] for item in getNamesEditByColums(db, file, code) if
            item[2] in types]


def make_selOneOpt(self, bd, form, rtable, lkp_field):  # make value for select one and multiple in jqgrid
    mySession = self.request.dbsession
    vals = {}
    result = mySession.execute(
        'select %s as qc, %s as qd from %s.%s;' % (
        lkp_field, lkp_field.replace("_cod", "_des"), self.user.login + "_" + bd, rtable))
...
def getNamesEditByColums(db, file, code):  # create available list of columns for editing online
    ...
    for i, x in enumerate(tree.find("tables/table")):
        ...
        if "odktype" in x.attrib:
            ...
            if x.attrib["odktype"] not in ["barcode","deviceid","start","end"] :
                if x.attrib["name"] not in ["rowuuid", "surveyid","originid",""]:#campos del formulario que no tienen que aparecer
                    #print x.attrib["name"]
                    row.append(x.attrib["name"])
                    row.append(x.attrib["desc"])
                    if "isMultiSelect" in x.attrib:
                        row.append("select")
                        rtable, rfield = getRTable(x.attrib["name"], x.attrib["multiSelectTable"], tree)
                        row.append(rtable)
                        row.append(rfield)
...
def fillDataTable(self, db, form, columns, file, code):
    ret = {"colNames": [], "data": [], "colModel": [],"projectMeta":[{"project_code":db,"form":form,"code":code,"user":self.user.login}]}
    sql = "select "
    ...
    for col in columns:
        ...
        if col[0] == "surveyid" or col[0] == "qst162":
            ret["colModel"].append(
                {"align": "center", "frozen": True, "label": col[1], "name": col[0], "index": col[0], "editable": False,
                 "width": 75,
            ...
            if "select1" in col[2]:  # list select type
                ret["colModel"].append(
                    {"align": "center", "label": col[1], "name": col[0], "index": col[0], "editable": True,
                     "formatter": "select", "edittype": "select", "validation": col[5],
                     "editoptions": {"multiple": False, "value": make_selOneOpt(self, db, form, col[3], col[4])}})
                ...
                if "select" in col[2]:  # list select multiple
                    ret["colModel"].append(
                        {"align": "center", "label": col[1], "name": col[0], "index": col[0], "editable": True,
                         ...
                         "editoptions": {"multiple": True, "value": make_selOneOpt(self, db, form, col[3], col[4])}})
                else:
    ...
    sql = sql[:-1] + ' from %s.%s_geninfo order by surveyid;' % (self.user.login + "_" + db, form.upper()+code)
    ...
    for res in result:
        ...
        for r in zip(result._metadata.keys, res):
            if str(r[0]) in get_FieldsByType(['select12', 'select'], db, self, file, code):
...
def update_edited_data(self, db, form, data, file, code):
    ...
    for row in data:
        if row['flag_update']:
            query_update = 'update %s.%s_geninfo set ' % (self.user.login + "_" + db, form.upper()+code)
            del row['flag_update']
            for key in row:
                val = ''
