...
from ..processes import create_packages_with_r,getPackages,projectHasCombinations,projectHasPackages,setRegistryStatus
from ..processes import generateRegistry, numberOfCombinationsForTheProject, searchTechnologiesInProject
            ...
            if stage == 1:
                ...
                if (self.request.method == 'POST'):
                    for key in self.request.params.keys():
                        if key.find("remove") >= 0:
                            ...
                            setCombinationStatus(self.user.login,projectid,id,0,self.request)
                        if key.find("add") >= 0:
                            id = int(key.replace("add",""))
                            setCombinationStatus(self.user.login, projectid, id, 1, self.request)
                    formdata = self.getPostDict()
                    scrollPos = int(formdata["scroll"])

                createCombinations(self.user.login, projectid, self.request)
                techs,ncombs,combs, = getCombinations(self.user.login,projectid,self.request)
                ...
                for comb in combs:
                    if pos <= len(techs):
                        ...
                        combArray.append({'ncomb': comb["comb_code"]-1,'comb_usable':combs[pos2-1]["comb_usable"], 'elements': list(elements)})
                        elements = []
                        elements.append({'alias_id': comb["alias_id"] ,'alias_name': comb["alias_name"]})
                        pos = 2
                    pos2 += 1
                ...
                return {'activeUser': self.user,'projectid':projectid,'techs':techs,'combArray':combArray,'stage':stage, 'csrf':self.request.session.get_csrf_token(),'scrollPos':scrollPos,'registryCreated':False}
            if stage == 2:
                if not projectHasCombinations(self.user.login, projectid, self.request):
                    self.returnRawViewResult = True
                    return HTTPFound(location=self.request.route_url('combinations',
                                                              _query={'stage': 1},projectid=projectid))
                ...
                return {'activeUser': self.user, 'projectid': projectid,'ncombs':combArray ,'packages':packages,'stage': stage,'registryCreated':False, "tech":getTech(self.user.login,projectid,self.request)}
            if stage == 3:
                if not projectHasCombinations(self.user.login, projectid, self.request):
                    self.returnRawViewResult = True
                    return HTTPFound(location=self.request.route_url('combinations', _query={'stage': 1},projectid=projectid))
                if not projectHasPackages(self.user.login, projectid, self.request):
                    self.returnRawViewResult = True
                    return HTTPFound(location=self.request.route_url('combinations', _query={'stage': 2}, projectid=projectid))

                startTheRegistry(self, projectid)

                return {'activeUser': self.user, 'projectid': projectid,'registryCreated':True}
...
def startTheRegistry(self, projectid):
    ...
    create_packages_excell(self.request, self.user.login, projectid, packages, getTech(self.user.login,projectid,self.request))

    locale = self.request.locale_name
    create_fieldagents_report(locale,self.request,self.user.login,projectid, getProjectEnumerators(self.user.login, projectid, self.request))

    numberOfCombinations = numberOfCombinationsForTheProject(self.user.login, projectid, self.request)
    ...
    for plugin in p.PluginImplementations(p.IPackage):
        plugin.after_create_packages(self.request, self.user.login, projectid, 'create_packages', ncombs, packages)