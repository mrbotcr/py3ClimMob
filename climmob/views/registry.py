...
from ..processes import addRegistryGroup,addRegistryQuestionToGroup,\
    deleteRegistryGroup,modifyRegistryGroup,getRegistryGroupData,getRegistryQuestions,\
    saveRegistryOrder,getRegistryGroupInformation,availableRegistryQuestions, \
    generateRegistryPreview,setRegistryStatus,getProjectProgress
...
class modifyRegistrySection_view(privateView):
    def processView(self):
        ...
        if not projectExists(self.user.login, projectid, self.request):
                ...
                if 'btn_modify_group' in self.request.POST:
                    ...
                    if data['section_name'] !='':
                        if data['section_content'] !='':
                            ...
                            error_summary = {'sectiondescription': self._("The description of the group cannot be empty.")}
                    else:
                        error_summary = {'sectionname': self._("The name of the group cannot be empty.")}
            else:
            ...
            return {'activeUser': self.user, 'projectid': projectid, 'data':self.decodeDict(data), 'error_summary': error_summary }
...
class deleteRegistrySection_view(privateView):
    def processView(self):
        ...
        if not data:
            ...
            data = getRegistryGroupInformation(self.user.login, projectid, groupid, self.request)
            if self.request.method == 'POST':
                deleted,message = deleteRegistryGroup(self.user.login,projectid,groupid,self.request)
                if not deleted:
                    error_summary = {'dberror': message}
                    self.returnRawViewResult = True
        ...
        return {'activeUser': self.user, 'projectid': projectid, 'data':data, 'error_summary': error_summary, 'groupid':groupid }
...
class newRegistryQuestion_view(privateView):
    def processView(self):
        ...
        if not projectExists(self.user.login, projectid, self.request):
                ...
                if 'btn_add_question_to_group' in self.request.POST:
                    ...
                    for key in postdata.keys():
                        if key.find("CHK") >=0:
                            ...
                            addq, message = addRegistryQuestionToGroup(data, self.request)
                            if not addq:
                                error = True
                    if not error:
                        self.returnRawViewResult = True
            ...
            return {'activeUser': self.user, 'UserQuestion': availableRegistryQuestions(self.user.login,projectid,self.request),
                    'QuestionsOptions': QuestionsOptions(self.user.login, self.request),'projectid':projectid,"groupInfo":getRegistryGroupData(data, self),'Categories': getCategories(self.user.login,self.request)}
...
class registryEnketo_view(privateView):
    def processView(self):
        ...
        if not projectExists(self.user.login, projectid, self.request):
            ...
            path = os.path.join(self.request.registry.settings['user.repository'],
...
class registryPreview_view(privateView):
    def processView(self):
        ...
        if not projectExists(self.user.login, projectid, self.request):
            ...
            generated,file = generateRegistryPreview(self.user.login, projectid,self.request)
            if generated:
                file = os.path.basename(file)
                pathToXML = self.request.route_url('registryenketo', projectid=projectid,file=file)
            else:
                pathToXML = ""
...
class closeRegistry_view(privateView):
    def processView(self):
        ...
        if not projectExists(self.user.login, projectid, self.request):
            raise HTTPNotFound()
        redirect = False
        progress, pcompleted = getProjectProgress(self.user.login, projectid, self.request)
        if self.request.method == 'POST':
...
class registry_view(privateView):
    def processView(self):
        ...
        if not projectExists(self.user.login, projectid, self.request):
            ...
            if self.request.method == 'POST':
                if 'saveorder' in self.request.POST:
                    ...
                    if not questionWithoutGroup:
                        modified,error = saveRegistryOrder(self.user.login,projectid,newOrder,self.request)
                    else:
            ...
            return {'activeUser':self.user,'data':data,'finalCloseQst':finalCloseQst,'error_summary':error_summary,'projectid':projectid}
