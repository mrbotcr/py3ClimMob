...
from ..processes import projectInDatabase,addProject,getProjectData,modifyProject,projectExists,deleteProject, changeTheStateOfCreateComb
...
class newProject_view(privateView):
    def processView(self):
        ...
        if (self.request.method == 'POST'):
            if 'btn_addNewProject' in self.request.POST:
                ...
                if int(dataworking['project_numobs']) > 0:
                    if dataworking['project_cod'] != '':
                        ...
                        if not exitsproject:
                            ...
                            if not added:
                                ...
                                newproject = True
                                self.request.session.flash(self._('The project was created successfully'))
                        else:
                    ...
                    error_summary = {'observations': self._("The number of observations must be greater than 0.")}
        ...
        return {'activeUser': self.user,'indashboard':True,'dataworking': dataworking,'newproject':newproject,'error_summary':error_summary}
    ...
    def processView(self):
        ...
        if (self.request.method == 'POST'):
            if 'btn_addNewProject' in self.request.POST:
                ...
                modified,message = modifyProject(self.user.login,projectid,data,self.request)
                if not modified:
                    error_summary = {'dberror': message}
                else:
                    newproject = True
                    self.request.session.flash(self._('The project was modified successfully'))
        ...
        return {'activeUser': self.user,'indashboard':True,'data':data,'newproject':newproject,'error_summary':error_summary}
...
class deleteProject_view(privateView):
    def processView(self):
        ...
        return {'activeUser': self.user, 'redirect': redirect,'data':data,'error_summary':error_summary}
...
class ProjectQRView(privateView):
    def processView(self):
        ...
        path = os.path.join(self.request.registry.settings['user.repository'], *[self.user.login, project_id, 'tmp'])
