...
from ..processes import getUserLog,getUserStats,getCountryList,\
    getSectorList,otherUserHasEmail,updateProfile,addToLog,getUserPassword,changeUserPassword
...
class profile_view(privateView):
    def processView(self):
        ...
        return {'activeUser': self.user, "activities": activities,'userstats':userstats}
    ...
    def processView(self):
        ...
        if self.request.method == 'POST':
                ...
                if data["user_fullname"] != "":
                    if data["user_email"] != "":
                        if data["user_organization"] != "":
                            if not otherUserHasEmail(self.user.login, data["user_email"],self.request):
                                updated,uerror = updateProfile(self.user.login, data,self.request)
                                if updated:
                                    ...
                                    addToLog(self.user.login, 'PRF', "Updated profile",self.request)
                                    profileUpdated = True
                                else:
            ...
            if 'changepass' in self.request.POST:
                ...
                if user.check_password(self.request.POST.get('user_password1', ''),self.request):
                    if self.request.POST.get('user_password2', '') != "":
                        if self.request.POST.get('user_password2', '') == self.request.POST.get('user_password3', ''):
                            if changeUserPassword(self.user.login, self.request.POST.get('user_password2', ''),self.request):
                                addToLog(self.user.login, 'PRF', "Changed password",self.request)
                                passChanged = True
                            else:
        ...
        return {'activeUser': self.user, 'userstats': userstats, 'data':  self.decodeDict(self.user.userData), 'error_summary': error_summary,
                'passChanged': passChanged, 'profileUpdated': profileUpdated,'countries':getCountryList(self.request),
