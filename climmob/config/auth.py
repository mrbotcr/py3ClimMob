...
def getUserByApiKey(apiKey, request):
    res = None
    result = request.dbsession.query(userModel).filter_by(user_apikey = apiKey).filter_by(user_active= 1 ).first()
    if not result is None:
        res = User(mapFromSchema(result))
...
def getUserByEmail(email, request):
    result = request.dbsession.query(userModel).filter_by(user_email = email).filter_by(user_active= 1 ).first()
